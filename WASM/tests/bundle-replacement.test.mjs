...
await test("another author replaces a chat owner atomically, with its own scopes and history", async () => {
  const n = await node();
  try {
    const old = await n.shell.loadBundleBlob(bundle(alice, "chat", 50, {
      protocols: ["chat", "old-only"], services: ["chat-local"], guestRequires: ["fs", "node"],
    }));
    await old.fs.put("private", Uint8Array.of(42));
    const next = await n.shell.replaceBundle(old.key, bundle(bob, "other-chat", 1, {
...
await test("explicit replacement still asks app admission and enforces revocation and freshness", async () => {
  ...
  try {
    ...
    allowed = false;
    await assert.rejects(n.shell.replaceBundle(old.key, bundle(bob, "chat")), /admission/);
    allowed = true;
    n.store.set(bob.id, "chat", 5);
    await assert.rejects(n.shell.replaceBundle(old.key, bundle(bob, "chat", 4)), /downgrade/);
    n.store.revoke(bob.id);
    await assert.rejects(n.shell.replaceBundle(old.key, bundle(bob, "chat", 6)), /revoked/);
    await assert.rejects(n.shell.replaceBundle("absent", bundle(carol, "chat")), /not installed/);
...
await test("a replacement cannot take unrelated claims or merge two installed identities", async () => {
  ...
  try {
    const old = await n.shell.loadBundleBlob(bundle(alice, "chat", 1, { services: ["chat"] }));
    const other = await n.shell.loadBundleBlob(bundle(bob, "other", 1, { protocols: ["reserved-peer"], services: ["reserved"] }));
    for (const extra of [{ services: ["reserved"] }, { protocols: ["reserved-peer"] }])
      await assert.rejects(n.shell.replaceBundle(old.key, bundle(carol, "candidate", 1, extra)), /already held/);
    await assert.rejects(n.shell.replaceBundle(old.key, bundle(bob, "other", 2)), /identity.*already installed/);
...
await test("stale replacement refuses concurrent replacement, update, uninstall, and close", async () => {
  for (const action of ["replace", "update", "uninstall", "close", "revoke-candidate", "claim-race"]) {
    ...
    try {
      ...
      const pending = n.shell.replaceBundle(old.key, bundle(bob, "chat", 1, { services: ["chat", "contested"] }));
      const refused = assert.rejects(pending, /target changed|node is closed|revoked|already held/);
      await waiting;
      if (action === "replace") await n.shell.replaceBundle(old.key, bundle(carol, "chat", 1, { services: ["chat"] }));
      if (action === "update") await n.shell.loadBundleBlob(bundle(alice, "chat", 2, { services: ["chat"] }));
      ...
      if (action === "claim-race") await n.shell.loadBundleBlob(bundle(carol, "neighbor", 1, { services: ["contested"] }));
      release();
      await refused;
...
await test("cross-author replacement works at the slot ceiling", async () => {
  const n = await node();
  try {
    for (let i = 0; i < DEFAULT_MAX_APP_SLOTS; i++) await n.shell.loadBundleBlob(bundle(alice, `app${i}`));
    const oldKey = appKeyFor(alice.id, "app0");
    await n.shell.replaceBundle(oldKey, bundle(bob, "replacement"));
    await assert.rejects(n.shell.loadBundleBlob(bundle(carol, "overflow")), /max app slots|already holds/);
...
await test("transport author can change only through replacement of its current owner", async () => {
  ...
  try {
    ...
    await assert.rejects(n.shell.loadBundleBlob(linkBundle(alice, 2)), /explicit replacement/);
    await assert.rejects(n.shell.loadBundleBlob(linkBundle(bob)), /explicit replacement/);
    const app = await n.shell.loadBundleBlob(bundle(carol, "chat"));
    await assert.rejects(n.shell.replaceBundle(app.key, linkBundle(bob)), /explicit replacement/);
    const next = await n.shell.replaceBundle(oldKey, linkBundle(bob));
    ...
    await assert.rejects(n.shell.replaceBundle(next.key, linkBundle(alice, 9)), /downgrade/);
    n.store.revoke(alice.id);
    await assert.rejects(n.shell.replaceBundle(next.key, linkBundle(alice, 11)), /revoked/);
    assert.equal(n.shell.resolve("_net"), next.key);
    await assert.rejects(n.shell.loadBundleBlob(bundle(bob, "transport", 2)), /use replaceBundle/);
    await n.shell.replaceBundle(next.key, bundle(carol, "offline", 1, { services: ["offline"] }));
  ...
  try {
    const app = await offline.shell.loadBundleBlob(bundle(alice, "chat"));
    await assert.rejects(offline.shell.replaceBundle(app.key, linkBundle(bob)), /explicit replacement/);
