...
import { type RawByteStream, type TransportCrypto } from "../core/socket-seam.js";
...
abstract class WsChannelBase extends BufferedChannel {
    protected readonly stream: RawByteStream;
    ...
    constructor(stream: RawByteStream, expectMasked: boolean) {
        super();
        this.stream = stream;
    ...
    stop(graceful: boolean): void {
        if (graceful) {
            try {
                this.stream.write(encodeFrame(WS_OPCODES.OP_CLOSE, CLOSE_NORMAL, this.mask()));
            }
    ...
    onData(chunk: Uint8Array) {
        ...
        if (!this.opened) {
            ...
            merged.set(this.handshake, 0);
            merged.set(chunk, this.handshake.length);
            this.handshake = merged;
            let consumed;
            try {
                consumed = this.tryHandshake(this.handshake);
            }
            catch {
                this.fail();
                return;
            ...
            if (consumed < 0) {
                if (this.handshake.length > MAX_WS_HANDSHAKE)
                    this.fail();
                return;
            }
            ...
            if (rest.length)
                this.feedFrames(rest);
            return;
        }
        this.feedFrames(chunk);
    }
    feedFrames(chunk: Uint8Array) {
        ...
        try {
            this.parser.push(chunk, (f) => {
                if (closed || this.dead)
                    return;
                if (f.opcode === WS_OPCODES.OP_BINARY)
                    this.deliver(f.payload);
                else if (f.opcode === WS_OPCODES.OP_PING)
                    this.stream.write(encodeFrame(WS_OPCODES.OP_PONG, f.payload, this.mask()));
                else if (f.opcode === WS_OPCODES.OP_CLOSE)
                    closed = true;
            });
        }
        catch {
...
export class WsServerChannel extends WsChannelBase {
    ...
    mask(): Uint8Array | null { return null; }
    tryHandshake(buf: Uint8Array): number {
        ...
        if (!key)
            throw new Error("ws: missing Sec-WebSocket-Key");
        this.stream.write(utf8.encode("HTTP/1.1 101 Switching Protocols\r\n" +
            "Upgrade: websocket\r\nConnection: Upgrade\r\n" +
...
export class WsClientChannel extends WsChannelBase {
    sodium;
    expectAccept;
    constructor(stream: RawByteStream, host: string, port: number, sodium: TransportCrypto) {
        ...
        stream.write(utf8.encode(`GET / HTTP/1.1\r\nHost: ${host}:${port}\r\n` +
            "Upgrade: websocket\r\nConnection: Upgrade\r\n" +
            `Sec-WebSocket-Key: ${key}\r\nSec-WebSocket-Version: 13\r\n\r\n`));
    }
    mask(): Uint8Array { return this.sodium.randombytes_buf(4); }
    tryHandshake(buf: Uint8Array): number {
        const sep = indexOfCRLFCRLF(buf);
        if (sep < 0)
            return -1;
...
function indexOfCRLFCRLF(b: Uint8Array) {
...
function headerValue(head: string, name: string) {
