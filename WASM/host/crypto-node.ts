...
//
// Use it when running on Node / Bun / Deno (with Node compat). A browser page reaches
// the runtime through `./shell-core` and readies its own crypto (docs/EXPORTS.md).
// Everything here needs the npm package or a local file read, which is exactly what
// makes it Node-only; nothing about the module table or the shell is target-specific,
// so neither lives here beyond the convenience re-export at the bottom.
...
/** Ready the whole crypto surface and return the one shared instance: the bundled
 *  sumo libsodium with ML-DSA-65 and ML-KEM-768 already mixed on. Apps (and the
 *  host) reuse this instance rather than each importing their own copy of the
 *  crypto library (README §12.1). It is still a libsodium-shaped object — the PQ
 *  primitives answer to libsodium-style names (pq.ts, kem.ts) — which is why the
 *  callers all bind it to `sodium`. */
 ...
 *  against. Requires `ensureCrypto()` first, like every other call here. */
