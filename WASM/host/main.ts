...
import { createShell, KernelHost, isAdmissionRejected, type RealmFactory, type Shell as CoreShell, type KernelTable, type ShellSodium } from "./shell-core.js";
...
/** Load a 32-byte secret from a hex file — the master seed (`--key`) and the
 *  deployment secret (`--contact-secret`) are the same shape, read the same way,
 *  and fail the same way when they are not. */
function loadHex32(path: string, label: string): Uint8Array {
    const k = fromHex(readFileSync(path, "utf8").trim());
    if (k.length !== 32)
        throw new Error(`${label} must hold 32 bytes of hex (got ${k.length})`);
    return k;
}
...
function loadNodeKeys(sodium: Sodium, keyPath: string): NodeKeys {
    if (existsSync(keyPath)) {
        return deriveNodeKeys(sodium, loadHex32(keyPath, "--key"));
    }
...
function loadContactSecret(path: string | undefined): Uint8Array | undefined {
    return path ? loadHex32(path, "--contact-secret") : undefined;
}
export async function main(): Promise<void> {
    ...
    const shell = await boot({
        ...
        listen: args["listen"] ? parseHostPort(str(args, "listen")!, { defaultHost: "0.0.0.0", allowEphemeral: true }) : undefined,
        wsListen: args["ws-listen"] ? parseHostPort(str(args, "ws-listen")!, { defaultHost: "0.0.0.0", allowEphemeral: true }) : undefined,
