...
// quickjs-ng amalgam only provides quickjs.h and quickjs-libc.h. The build
// script passes -I with the fetched amalgam's headers, so the include is a
// bare name here (the upstream file spelled out its repo-relative path).
...
static void *qts_array_buffer_realloc(JSRuntime *rt, void *opaque, void *ptr, size_t size) {
  if (size == 0) {
    free(ptr);
    return NULL;
  }
...
JSValue *QTS_NewArrayBuffer(JSContext *ctx, JSVoid *buffer, size_t length) {
  return jsvalue_to_heap(
      JS_NewArrayBuffer(ctx, (uint8_t *)buffer, length, 0, qts_array_buffer_realloc, NULL, false));
}
