...
// §14.1) with scripts/bundle-author.ts's authorBundle and the verifier's shared validation.
...
// identifies the selected transport (§12.5); a rebuild with a different key is a new
// author. The 0x02 id is a key-set hash, so the ML-DSA half derives from the same seed —
...
async function main() {
  ...
  // from the SAME seed (scripts/bundle-author.ts `hybridAuthorKeysFromSeed`), so a rebuild with the
