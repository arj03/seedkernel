// bundle-native-host.mjs — assemble tsc-built host modules into one global script the native
...
//
// Usage: node bundle-native-host.mjs <out.js> <Export1,Export2,...> <file1.js> <file2.js> ...
import { readFileSync, writeFileSync } from "node:fs";
import { sourceStamp } from "./source-stamp.mjs";

const [, , out, exportsCsv, ...files] = process.argv;
if (!out || files.length === 0) {
  console.error("usage: bundle-native-host.mjs <out.js> <Exports,csv> <file...>");
  process.exit(1);
}
...
for (const f of files) {
  ...
  for (const n of names) {
    if (declared.has(n)) {
      console.error(`bundle-native-host: ${f} and ${declared.get(n)} both export "${n}" — one name, two definitions`);
      process.exit(1);
    }
    declared.set(n, f);
  }
...
for (const n of exportNames) {
  if (!declared.has(n)) {
    console.error(`bundle-native-host: no module in this bundle exports "${n}"`);
    process.exit(1);
  }
}
const slots = [...declared.keys()];
// The sources this artifact derives from, so a consumer can tell whether it still matches
// the tree (scripts/source-stamp.mjs). One comment line, read by name rather than by
// position: `//@sources {"<path>":"<sha256>",...}`, paths relative to WASM/.
const wrapped = `"use strict";\n// GENERATED by scripts/bundle-native-host.mjs — do not edit.\n`
  + `//@sources ${JSON.stringify(sourceStamp(files))}\n`
  + `(function () {\n`
