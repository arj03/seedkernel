...
JSModuleDef *QJS_ModuleLoader(JSContext *ctx, const char *module_name, void *opaque)
{
    ...
    /* JS_UNDEFINED for the import attributes quickjs-ng v0.16 added: this loader is
       registered through JS_SetModuleLoaderFunc, whose callback shape has no attributes
       to forward. Reaching them would mean JS_SetModuleLoaderFunc2, and nothing here
       imports with attributes — the confined realm loads no modules from disk at all. */
