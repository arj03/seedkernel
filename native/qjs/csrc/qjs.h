/* The engine, by include path rather than by relative directory: upstream keeps its
   checkout inside the shim's own folder, while here it is a throwaway clone under
   .build/ that build-qjs.sh points the compiler at. */
#include <quickjs.h>
