...
The `link` names are the **raw-link** capability. Raw events arrive directly at the one slot owning that capability; what a transport provides back is an ordinary local service claim. The occupant's observations about a platform-owned *socket* are returns from the event that caused them, never guest-callable host names, because a name would let the occupant say them about a link it did not observe them on: `linkBytes(linkId, bytes)` returns the 32-byte peer this read authenticated, or nothing; `linkClosed(linkId)` returns the occupant's one-byte reason. Neither return carries a link id, so it cannot redirect a transition to another socket, and a stale return after close, reset or handover is discarded. A request the occupant decoded off those bytes travels the other way, as its own `link/deliver` call — it names no link, so the argument that keeps the two observations above off the seam does not reach it. Deliveries reach only what a peer may reach, so a bundle's own `services` claim is not among them (§12.10): the *same shell* routing that refuses a peer's call to a local service name decides the delivery.
