...
| Name | Serves | |
| --- | --- | --- |
| `node/sign`, `node/identity`, `node/random` | the node key and the entropy source — what the *host* owns | |
...
7. Only now may the guest run (§12.8): a realm (§12.3) over a cap-bridge restricted to `guest.requires`, loaded with `op preamble ‖ const BUNDLE ‖ const APP = merge(guest.config, operator config) ‖ guest source`.
