...
**A host may serve claims of its own,** through optional `createShell({ claims })` — exact names, no wildcard and no fall-through, sharing one lookup and one conflict rule with the bundles' (a bundle contesting a platform name is refused as it would be against another bundle). Registering a `_`-led one is the deliberate act that opens that name to peers — seedchat's `_offer`, which carries a bundle between two browsers before either has an app that could receive it, is host code saying so. Attribution follows the path rather than the name: a peer key inbound, a caller app key from a co-resident guest.
