...
| --- | --- | --- | --- |
| `linkOpen` | 8 | `[linkId u32][weDialed u8][stream u8][listener text][expectPeerId blob][remoteAddr text]` | Ignored by the driver |
...
**The raw-link binding has one occupant, like a claim.** The driver has one event sink, so two link-capable slots are not a composition: the second would take the node's sockets off the first while leaving its claims and realm in place — installed, routed, and silently off the network. So an install reaching `link` while another slot holds the binding is *refused*, on the same rule and at the same two points as a contested claim. The holder changes only by being uninstalled, or by being the `replaces` of the install that takes over. No owner token is needed: the shell wires `rawNet` only into that slot and commits a replacement synchronously.
