...
**Bounded, not merely confined.** Zero authority answers "what can this guest reach"; it says nothing about "how much of this node can it consume". The answer is one rule rather than a list of defences: **every host-side byte caused by untrusted input has an owner, from creation until destruction.** When data moves, the receiving owner reserves capacity before the previous one releases it, so there is never an interval where retained bytes are charged to nobody. A caller supplies no charging flag and no operation name earns an exemption — a name may *tighten* what an owner admits (`fs/put` meets a storage quota) but never relax it, and the bound is enforced by whoever owns the resource, never by the dispatcher that routed the call. Each owner is finite and has a complete teardown path, and each is bounded in **time** as well as size: a release path nothing is committed to calling bounds only until the freeing event fails to happen. The owners below each default to a real number, so a shell that configures none still gets a bounded guest:
...
- **The node's clock, against work a guest hands itself** — a fired deadline is the one initiation a guest can issue to *itself*. Every other entry into a realm was paid for by an owner that bounds it — a peer's frame by the driver's read window, a cross-realm call by the caller's active-call registry, a host loopback by its own — so the per-invocation ceiling times those bounded populations is their total. `timer/arm(id, 0, …)` from inside the timer entrypoint has no such upstream: the guest supplies its own successor and each takes a fresh full budget, which is right per invocation and, in aggregate, one installed app holding the node's only thread with no peer involved. So each realm's timer table meters the wall clock its fired invocations occupy — **one** period for however many are outstanding, since a serialized realm is occupied once — banks one whole invocation of that realm's budget, and earns the clock back at `1 / SELF_INITIATED_CLOCK_DIVISOR` (16, twice `DEFAULT_MAX_APP_SLOTS`). A deadline that comes due with the share spent is **slipped**, never failed and never dropped: failing would turn a share held against the node's other slots into an error an honest app must handle, and dropping it would lose a keepalive the app is entitled to. What is metered is the clock those fires occupy and never how many there are, so the transport's own shape — a cheap deadline per link, all coming due at once — meets none of it, while a guest that spins simply fires more slowly. A full node's summed share is half its clock; the other half stays with the peers and the host loop no guest can grant itself, and both terms are added up in `tests/verify-hardening.mjs` beside the memory sum.
