...
"Capabilities" from here on mean one thing: the **bundle cap domains** (§12.2, §12.4) — coarse names (`node`, `net`, `fs`, `clock`, `timer`, plus the two only a slot occupant may declare, `link` and `transport`) that a bundle's signed manifest grants to the app's confined JS *guest*. They answer "may this *app's guest* reach this backend at all?" (WASM handlers, by contrast, carry no capabilities at all — a pure transform reaches nothing but the input it is handed and the output it returns, §4.2.)

**Only authorities appear there.** A crypto primitive is a function of bytes the guest already holds, so it reaches nothing and there is nothing to grant: the guest calls it by name through one ungated op and the host resolves it in a catalog (§12.2). The same rule exempts `module/call`: an app's modules are its own bundle's code, installed and verified with it, so calling one reaches nothing the guest does not already hold — the scope (one app's map) is the shape, not a grant. What a manifest declares beside `caps` is `primitives` — the names it calls — which is a *compatibility* declaration, so a host missing one refuses the load by name instead of failing at the guest's first call.
...
The **capability domains** a manifest declares (§12.4) are the *prefixes* of the names above — the coarse, human-auditable vocabulary ("this app reaches net + fs"), not a list of op numbers. A granted domain IS its prefix: the bridge refuses any `host.call` whose first path component is not one of the declared domains, and neither the `crypto/` nor the `module/` prefix is in any of them:
...
`crypto/*` and `module/call` appear in no row: there is nothing to grant, so a manifest never asks for either — both prefixes are exempt from the gate by a rule. The `crypto/` catalog is fixed host-side; the module map is the bundle's own, declared in its manifest's `modules[]` (§12.4). What a manifest *does* declare is `guest.primitives` (§12.4) — the bare primitive names (minus the prefix) it calls, checked against the host's catalog at load.
