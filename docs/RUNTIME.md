...
**A host may serve reserved ids of its own,** through optional `createShell({ answer })`, consulted on inbound frames before claims. `null` falls through. The hook is inbound-only; a co-resident guest calls a reserved claim and is attributed by a host-derived caller id.
...
**Replacing the transport is ordinary slot replacement, not a protocol.** The same `(author, app)` loads a complete candidate offside, including its private modules, realm and scopes. After it stands and its freshness mark persists, the shell synchronously replaces that identity's slot and claims. A different identity contesting `_net` is rejected rather than shadowing it. There is no handover: the socket driver retains the address book and listeners, while the incoming guest reads their configuration through its side-effect-free `link/config` capability — same listening port, same node identity. Live links do not survive because their session keys were private to the outgoing realm, so an upgrade reconnects.
