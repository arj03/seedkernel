...
**The node's immutable facts use the ordinary config path, not a privileged invocation.** `TransportHost.initialConfig()` returns exactly `peerId`, `networkKey`, and `contactSecret` as JSON; the shell merges them into the link occupant's installation-local `LOCAL` object before `standRealm`, and those three host-owned keys win same-named values supplied by the load. Their binary values use the same hex spelling as peer references. They are constructor input, delivered once and not re-readable. The address book is mutable node state and is not among the facts: it arrives as `addr` events, replayed after publication (§12.10).
The **contact secret** is the one fact that is not read-only in practice, so it is re-delivered per link too: on every link the host hands over, the driver reads its current value and carries it in the `linkOpen` event (§12.6.3), so a node that rotates its secret gates new links immediately, with no re-install; only a transport **upgrade** is a re-load (§12.10). A link the guest dials through `link/open` gates on the *peer's* secret instead, which the address book carries — the one place a deployment can name a secret that is not its own. The `LOCAL` value remains the fallback.
