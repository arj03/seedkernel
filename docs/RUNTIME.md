...
| `node/sign` | message bytes | 64-byte detached Ed25519 signature under the node identity, over `domain ‖ scope ‖ msg` — this slot's one scope (`slotSignScope`, below, §16.1): an app slot's own `author ‖ app`, an empty scope after `DOMAIN_link_scope` for the link slot. The host chooses what the name signs under; never guest-supplied |
...
```
manifest            the app's identity, claims and guest authority
...
| *(both `protocols[]` and `services[]`)* | | | Two **claim maps**, not one vocabulary: one owner per ACTIVE name *per map*, so no second identity can shadow an owner in either, while a replacement atomically takes over its predecessor's slot and whole claim set. A name may appear in both lists, which says "reachable by a peer *and* by a co-resident guest" — two reaches to one owner, which two maps express without a rule. A claim grants nothing — which list it sits in decides reach, not any leading character; no spelling is reserved to the kernel. |
...
**The policy needs no ownership state.** Identity comes from the verified `(author, app)` and a claim points to the resulting slot. The policy is a pure function of the bundle; module ownership cannot drift because modules never exist outside their slot.
...
`install(blob, options)` admits ordinary apps through `admit(v)`. Naming no `replaces`, it takes a FREE identity: a blob whose `(author, app)` is already installed is refused by name, and so is one requiring `link`. An absent app predicate is `denyAll`. `authorAllowlist`, interactive consent, and `admitAll` express app trust; further checks, such as sensitive claim owners, compose with them as ordinary functions.
...
**An install takes over nothing it did not name — not a claim, and not an identity.** Without `replaces` it lands only where nothing is standing; every takeover names its target, or uninstalls first. This makes ownership deterministic, keeps installation order from changing routing behind an already admitted app, and gives two concurrent installs of one app a refusal rather than a silent winner. The host-internal app key still allows different authors to reuse an `app` label; it does not namespace literal protocol or service claims.
