...
| Component | Size |
|---|---|
| `core/*.js` + hand-written `host/*.js` a page can reach — minified (`build-min`, excluding the embedded transport) | ~124 KB |
...
| QuickJS realm engine (the in-repo quickjs-ng 0.16.2 emscripten build, `quickjs/` — the same engine source the native loader compiles, §12.9) — only loaded when a bundle's guest runs (§12.3) | ~570 KB |
...
Chat is the smallest possible app: a confined JS **guest** over a single **restartable** module (§4). The guest is a handful of lines — its `handle` entrypoint forwards its input to the module by name through `host.call` and returns the render bytes — and the module is the transform: it does no I/O and no crypto, reading `senderPk ‖ chatType ‖ body` and writing the render bytes for the UI. Everything around it — authenticating the sender, moving frames, driving the iframe — is the shell's job, because a module has no reach of its own and a guest reaches the world only through `host.call` (§12.2).
...
The chat module never reaches the UI itself: it is a transform that *returns* render bytes, which the guest's `handle` passes back to the shell, which forwards them to the iframe by `postMessage`. The iframe is `sandbox="allow-scripts allow-forms"` with no same-origin access to the shell, so app-rendered content stays walled off from the shell's keys and peer state.
...
"Capabilities" from here on mean one thing: the **manifest requires** (§12.2, §12.4) — SERVICE-level `host.call` grants such as `node`, `fs`, `timer`, `link`. `link` requires explicit transport selection (§12.5). They answer "may this *app's guest* reach this backend at all?" (WASM modules, by contrast, carry no capabilities at all — a module reaches nothing but the input it is handed, its own private memory, and the output it returns, §4.2.)
...
**Raw-link event byte layout.** The socket driver encodes these events with `OpArgs` from `core/op-frame.ts`, and the shell prepends its host caller id. Each event arrives at the link occupant's `handle` as `[caller 32 zero bytes][opLen u8][op ASCII, opLen bytes][args …]`. The envelope and the following field order are kernel ABI: every replacement link occupant must accept them. There is no padding or terminator. Every `u32` is unsigned big-endian; `blob` is `[byteLength u32][bytes …]`, and `text` is a blob containing UTF-8 bytes (the length counts bytes). Boolean fields are one byte, `0` or `1`.
...
The first two cross every seam between the operator and the realm — CLI flag, `bootNodeShell()`, `bootShell`, `InstallOptions`, `RealmFactory` — because a bound the shell accepts but no target can set is a bound nobody has — one the realm factory takes and nothing upstream carries is dead, since the default applies and nothing can change it. `--guest-timeout 0` reads as "no budget", so disabling one is something an operator says rather than something a missing or mistyped flag does: a value that is not a whole number is refused, and `bootShell` refuses a heap limit below 1 byte or from 2³² up, or a budget under 1 ms, since the two engines would read those differently (the JS engine takes a heap limit of 0 as none).
...
- **The guest is the app; the modules are its library.** The guest is JS source for a QuickJS realm, not WASM — the loader's path ends in "instantiate WASM, bind," and the guest is what the shell runs. Modules are the restartable transforms the guest drives by name; a bundle may declare zero or many of them. Without the manifest the guest would have no signed identity at all.
- **The guest's authority has no other home.** A WASM module is a transform with no capabilities of its own (§4.2); the guest, by contrast, *does* reach I/O — through the guest seam — yet is not a module and has no table entry of its own, so the manifest's `guest.requires` is its entire capability declaration.
...
**One authenticated statement, one authorization.** The signatures commit to the manifest, guest and every module; `verifyBundle` proves the bytes, and one policy decision authorizes the whole set. The loader then constructs a complete slot offside and commits its claims atomically (§3.1). A live update is the same path with a higher version, naming the slot it retires so that what dies is a thing the operator chose rather than a coincidence of keys.
...
**Install algorithm** (`install`). The shell is host code, so failures here **throw to the operator** — §3's "an unbound name resolves to an empty response" is about resolving a name, not about loading a local file.
...
Admission (§12.4) runs after verification and before slot construction. Every bundle clears the host's gates — revocation, then freshness — before any app predicate, and again in the synchronous commit window, where the facts they read may have moved. Revocation comes first so a written-off key never reaches an interactive consent dialog. The gates are the shell's rather than the operator's because they are invariants, not posture: an `admitAll` node still has a downgrade guard.
...
**Revocation is host-side, and it is one action.** A module is a transform with no capability imports (§4.2), so nothing in the sandbox can reach the loader — there is no revoke-message, and revocation is not something a bundle can carry.
...
**The raw-link binding is a claim.** A host service with `events` (`core/domains.ts` — today only `link`) has an occupant, and the driver has one event sink, so two holders are not a composition: the second would take the node's sockets off the first while leaving its claims and realm in place — installed, routed, and silently off the network. Requiring such a service therefore claims it, in a third book beside `peerClaims` and `localClaims`, under the same rule and at the same two points: an install contesting the holder is *refused*, and the holder changes only by being uninstalled or by being the `replaces` of the install that takes over. One rule is the book's own: a *free* `link` is taken only by the boot's transport selection (§12.5), because it carries authority no app predicate grants. The driver follows the book — bound to the holder, released when nothing holds it — and the shell wires `rawNet` only into that slot, so no owner token is needed.
