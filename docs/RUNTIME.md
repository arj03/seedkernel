...
- **Outstanding host calls and their inputs** — at most 256 unresolved calls and 16 MiB of aggregate copied input per realm (`DEFAULT_MAX_OUTSTANDING_HOST_CALLS`, `DEFAULT_MAX_OUTSTANDING_HOST_CALL_BYTES`). Both leases last until settlement. The checks run before the payload copy crosses out of QuickJS, so neither many tiny fire-and-forget calls nor a handful of maximum-sized network deliveries can multiply host-side buffers and promises outside the confined heap.
...
- **`MAX_OUTBOUND_QUEUE_BYTES` (16 MiB) and `MAX_OUTBOUND_QUEUE_SLICES` (4096)** bound
  ...
  window to frames waiting on its ordered encryption chain; the Node adapter measures
  `writableLength` plus pending callbacks, the browser adapter measures `bufferedAmount`, and
  native counts its exact slice queue. Crossing either limit fails the whole link rather than
  dropping a write from the ordered stream. The slice limit also applies to the pre-open queue.
