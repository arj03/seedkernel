...
**One authenticated statement, one authorization.** The bundle is the *only* way code arrives. The signed manifest commits to every module's `genesisHash` (§5.1), and the loader verifies each `.wasm`'s bytes against it, then admits each verified module under the kernel name it *derives* from the manifest's signed `(app, name)` pair (§5.1) — a policy decision (§12.5) followed by `SetHandler`, recording `(author, bytes_hash)` where the author is the manifest author and the hash is the verified one. Admission touches no replay state, so an equal-version reload just re-binds cleanly — a reboot re-reading the same bundle installs the same modules again with no collision. A **live update** is not a separate mechanism: it is delivering a bundle whose manifest `version` is higher, which the freshness guard (below) requires and the same-author rule (§12.5) admits.
...
| --- | --- | --- | --- |
| `app` | string | **yes** | Names the coherent set. Three jobs: with `author_pk` it forms the `(author, app)` freshness key (see freshness below), it scopes the guest's signing namespace (`guestSignScope`, §12.2), and it is the first half of every module's kernel name (`modules[]` below). Non-empty. |
...
| `modules[]` | `{name, hash}` | yes | One entry per WASM module. `name` does four jobs at once: the module's file in the container (`<name>.wasm`), the key the guest addresses it by (`BUNDLE.modules`, below), how the loader reports it, and — with `app` — the kernel name it binds at, `"<app>:<name>"` (§5.1) — so it is unique within a manifest and restricted to `[A-Za-z0-9_-]`. `hash` is `genesisHash(wasm)` hex (the *same* value the loader records as the module's `bytes_hash` and a policy `modules` allowlist matches, §5.1). **There is no bind-name field:** the loader derives the name from two values the author already signed, so a manifest holds nothing that could point a module at an unexpected handler. |
...
6. **Admit** each verified module at `"<app>:<name>"`, derived from the manifest (§5.1). A module the policy refuses does not abort the load — the loader reports which modules were admitted, and the operator decides.
