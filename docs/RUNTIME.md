...
The `host/*.js` layer is the whole runtime: bundle slots and claim routing, raw net and fs seams, the guest seam, safe-js, verification, policy, and the transport driver. Pure modules remain target values owned privately by a slot. The transport protocol is signed content (`transport/src/*.js` plus `ws.wasm`); the first copy ships inside the artifact because a node has no network until it has a transport, and a later signed bundle can replace it. libsodium backs the primitive catalog, content hashing, and manifest verification. QuickJS is lazy: a shell with no installed slot never pays for a realm.
...
"Capabilities" from here on mean one thing: the **manifest requires** (§12.2, §12.4) — exact `host.call` grants such as `node/sign`, `fs/get`, `timer/arm`, `link/*`, `route/deliver`, and reserved local service ids. `link/*` requires the operator's `link` grant; `route/deliver` independently requires `route`. They answer "may this *app's guest* reach this backend or local service at all?" (WASM modules, by contrast, carry no capabilities at all — a pure transform reaches nothing but the input it is handed and the output it returns, §4.2.)
...
- **Raw links, structured service, and attributed delivery are independent.** `link/*` moves bytes over an opaque link id and binds platform link events directly to the slot holding that capability — one slot, since the driver has one event sink, so a second link-capable load is refused rather than allowed to take the sockets over (§12.10). The transport exposes its structured API under an ordinary local claim selected by composition (`_net` in the bundled setup). Inbound attributed requests use `route/deliver`, a separate `route` privilege accepting `(claim, opaque attribution, payload)` — separate because the submitter *writes* the attribution, which holding sockets does not earn. No claim spelling confers either privilege (§12.5, §12.10).
...
| `node/sign` | message bytes | 64-byte detached Ed25519 signature under the node identity, over `domain ‖ scope ‖ msg` — always this app's own scope (`appSignScope`, below, §16.1), unconditionally: gaining another privilege never changes what this name signs under, never guest-supplied |
...
| `link/config` | (empty) | static transport configuration: `[identity blob][network key blob][contact secret blob][limits u32 …][admitted peers blob]`. Immutable node facts only — the mutable address book is delivered separately, as `addr` events after the raw-link binding is published. The shape is versioned by `guest.abi`, so removing or reordering a field bumps it and appending one does not (§12.4). |
...
| `link/authenticated` | `[linkId u32][peer 32]` | (empty) — a link the host handed over (`openLink`) authenticated as `peer`, relayed to whoever passed the channel in |
| `link/down` | `[linkId u32][reason u8]` | (empty) — the same link's teardown, with the transport's reason code |
| `link/sign` | message bytes | 64-byte detached Ed25519 signature under the node identity, over `domain ‖ scope ‖ msg` where the scope is always this node's network scope (`linkSignScope`, below, §16.1) — `node/sign`'s twin, wired only for a slot reaching `link`, and a separate name rather than a second meaning for `node/sign` |
| `link/verify` | `[pk 32][sig 64][msg ..]` | `[ok u8]` — the same network `domain ‖ scope` applied host-side; `node/verify`'s twin, under the same conditions as `link/sign` |
| `route/deliver` | `[claimLen u8][claim utf8][attrLen u32][attribution][payload]` | the claimant's answer — **awaited**; empty when nothing wire-reachable serves the claim |
| `timer/arm` | `[id u32][ms u32]` | (empty) — fires the `timer` entrypoint |
| `timer/clear` | `[id u32]` | (empty) |
| *any `_`-led name* | opaque bytes, with the CALLER's 32-byte id prepended host-side | whatever the claiming realm's `handle` returned — **awaited**. A reserved local service id (§12.10), with no kernel-known spellings or roles; refused by name when nothing claims it |

The `link/*` names are the **raw-link** capability. Raw events arrive directly at the one slot owning that capability. What a transport provides back is an ordinary local service claim. Attributed inbound requests enter the claim table through `route/deliver`; that generic capability, not the service claim or raw-link grant, authorizes delivery — and it reaches only what a peer may reach, so a bundle's own `_`-led claim is not among them (§12.10).

Every `_`-led name and the six `fs/*` names genuinely round-trip: the guest `await`s them, and a fan-out is the guest's own `Promise.all` — the seam hands out real promises, so scatter-gather is the guest's own, not a host name. Every other name resolves to bytes without yielding, and **which side of that line a name sits on is the ABI**: moving one is exactly what `guest.abi` versions (§12.4), because the failure is otherwise silent — a guest that forgets the `await` reads a Promise as bytes. The host injects `__host_call(name, callId, payload)`: bytes complete a sync name inline; `null` means async under `callId`, settled later by `__netResolve`/`__netReject`. **`null` is reserved** — a sync name that returned null would hang the Promise forever. **No name re-enters the realm**: a socket write does not deliver during the write, an armed timer fires on a later turn, and a cross-realm call runs its callee on a later turn by construction, so a guest→guest call never runs the callee inside the caller's frame.
...
**The signing names are scoped, never raw, and gaining a privilege only ever ADDS an endpoint — never changes what an existing one means.** `node/sign` does not sign the guest's bytes as given: the host prepends a domain and a scope, both of its own choosing, and never reads the suffix. Every app, whatever else its slot reaches, gets `DOMAIN_guest ‖ author_pk ‖ app_len u8 ‖ app` (`appSignScope`), derived from the admitted manifest (§12.4) — the same `(author, app)` pair that keys freshness. A bundle reaching **`link`** additionally gets a SEPARATE name, `link/sign`, scoped to `DOMAIN_link_scope ‖ network_key` (`linkSignScope`) — never a second meaning for `node/sign` itself. That is what lets the AKE transcript signature be an ordinary `link/sign` call without touching what `node/sign` means for that same slot: the kernel owns the separation and nothing else, so the *format* signed under that scope — including the transport's own `DOMAIN_channel` tag — is bundle content, revisable in a bundle update, and the node key never enters the guest. Both scopes are one function of admitted facts (`slotSignScopes`), which is why each is the same on every load path: boot, an operator's `--bundle`, and the in-place update that replaces a standing slot alike. The domain-prefix family is disjoint (§14, §16.1), so a guest-obtained signature never verifies as a manifest; and distinct bundles derive disjoint scopes, so one app cannot sign in another's namespace. **Verification is scoped the same way, through the matching name.** `node/verify` and `link/verify` each take the verifying key in their argument bytes — `[pk 32][sig 64][msg …]` → `[ok u8]` — and the host applies the matching `domain ‖ scope` prefix, so an app checks a scoped signature by naming the key and never reconstructs the prefix the host owns. Each is an authority because its scope is host-derived: the guest asks "does this signature verify under *my* bundle's namespace?" (`node/verify`) or "under *this node's network*?" (`link/verify`), a fact it cannot state for itself. The raw `crypto/ed25519/verify` primitive stays, ungated like every pure transform, for callers verifying raw bytes. Every node running the same bundle derives the same scopes, which makes the signatures portable across a cohort. One consequence: rotating a bundle's author key changes its app scope and orphans previously signed objects, so an app anticipating that records its scope inside its own signed formats. §14 has the trust rationale.
...
| Name | Serves | |
| --- | --- | --- |
| `node/sign`, `node/verify`, `node/identity`, `node/random` | the node key, this app's own scoped signing/verification — unconditionally — and the entropy source — what the *host* owns | |
...
| `link/config`, `link/open`, `link/send`, `link/close`, `link/stat`, `link/authenticated`, `link/down`, `link/sign`, `link/verify` | **`link` privilege only** — bytes over an opaque link id, the platform's whole contribution to the network, the reports back to the binding that supplied one, and the node's network-scoped signing/verification (`node/sign`/`node/verify`'s twins, never a second meaning for them) | |
| `route/deliver` | **`route` privilege only** — submitting a request whose attribution the submitter writes | |
| *any `_`-led name* | another REALM, by the id it claims — no host authority at all, and carrying no privilege | |
...
**`Shell.close()` waits for in-flight calls.** Every entry into a slot's guest (`invoke`, `dispatch`, a handle's `invoke`) is chained onto one wait (`inFlight` in `shell-core.ts`), and `close()` defers realm disposal until that wait settles. A call parked mid-await — a repair pass waiting out an unreachable peer — must not resume into a freed QuickJS context. Uninstall and revoke dispose immediately; a handle held past that rejects by naming the slot rather than calling into a freed realm.

The first two cross every seam between the operator and the realm — CLI flag, `bootRuntime()`, `bootShell`, `LoadBundleOptions`, `RealmFactory` — because a bound the shell accepts but no target can set is a bound nobody has — one the realm factory takes and nothing upstream carries is dead, since the default applies and nothing can change it. `--guest-timeout 0` reads as "no budget", so disabling one is something an operator says rather than something a missing flag does.
...
| `protocols[]` | string[] | yes | The protocol or local-service ids this app serves (§12.10) — the app's **claims**, and the whole of routing: load assigns them to the completed slot and uninstall releases them. Claims are unique active names; a different identity cannot shadow an owner, while an update atomically replaces its own slot and claim set. A claim grants nothing. Ordinary ids use an alphanumeric first character; `_`-led ids are local cross-realm service names. No spelling is reserved to the kernel or implies a privilege. **Optional:** an initiator-only app claims nothing, and an absent list says exactly that. |
...
| `guest.abi` | integer | **yes** | Which host seam and preamble this guest was written against (`GUEST_ABI_VERSION`, §12.2). A guest declaring an ABI this host does not implement is refused by name — a legibility failure, like an unsupported signature suite, not an authenticity verdict. Required rather than defaulted: the default would have to be the oldest ABI, which is exactly the population a bump exists to catch, and a guest author who never considered the seam version is indistinguishable from one who meant the old one. The number tracks changes to the seam's *shape* — the naming scheme of `host.call`'s first argument, a name moving across the sync/async line, a payload framing change, the entrypoint protocol, or a preamble global changing meaning — not the appending of new names, which a guest that never calls them cannot notice. It is the version of every name it contains: within an ABI a name's meaning is fixed. Its reason for existing is that the failure it guards is silent: a guest calling `host.call("fs/get", k)` without `await` gets a Promise where bytes were expected and reads `undefined` — a wrong answer, not an error, and one no care at the call site turns into a loud one. A declared seam version makes it a refused load. |
| `guest.requires` | string[] | **yes** | **Exactly the authorities and local services this guest may call** (`AUTHORITY_CALLS`, §12.2, plus reserved ids). The seam grants these name by name; the shell wires only their backends. Ungated `crypto/*` and the bundle's own module names are not declarable. The list is closed and may be empty. `link/*` requires the operator's `link` grant; `route/deliver` independently requires `route`; reserved local service ids carry no privilege. |
...
**App configuration has two named provenances and one-app scope.** `APP` is exactly the signed `guest.config`; the host never merges into it. `LOCAL` is the optional JSON object supplied to this particular `loadBundleBlob(blob, { localConfig })` call. It is not retained on the shell and cannot reach the transport or another bundle loaded beside this one. Both are objects and both are always defined (absent ≡ `{}`), so a guest guards keys, never the binding. The runtime interprets neither and chooses no precedence: an app that supports installation-local tuning validates and combines `APP` and `LOCAL` itself, in guest code its own author signed. Host-owned resource policy — realm memory, execution deadlines, socket limits and admission — remains outside both.
...
**Every pure question about a bundle is a gate, and each gate sits where its question is decided.** Claim spellings are checked only for shape and uniqueness; none carries privilege. Privileges come solely from exact authority names in signed `guest.requires`, and admission decides whether the operator grants each one. In particular, `link/*` and `route/deliver` derive independent `link` and `route` privileges. Admission's own host gates (`notRevoked`, `freshVersion`) are composed around the operator predicate. A rule that refuses a bundle from outside that predicate would be a second decision point, and a second decision point is where "nothing has landed until the predicate says yes" stops being a property of the type and becomes a property of how carefully the load path is read.
...
A transport bundle reaches **both** privileges — `link/*` to hold the sockets, `route/deliver` to submit what it decodes off them — and a bundle reaching a privilege is judged by *every* grant it reaches, so a policy naming only `link` refuses it. A node whose policy is missing one says so at the boot (`no transport: the policy does not grant this bundle all required privileges`) and then runs with no network.
...
| `grants` | at least one of `authors` / `grants` | Who may hold each **privilege**, keyed on the privilege itself. The keys are the host's `PRIVILEGES` (derived from `AUTHORITY_CALLS`, `core/domains.ts`); a key that names none is refused at the boot, so an operator cannot misspell a grant into a node that holds nothing. Today there are two, both deliberate decisions apart from `authors` and never app code: `link`, the raw links the node's transport is built out of — the holder carries the node's whole network and signs the channel AUTH (§12.2) — and `route`, submitting an inbound request whose *attribution the submitter writes*, which is why it is not folded into `link`: holding sockets is not the same as being trusted to say which peer a request came from. A bundle reaching a privilege must be admitted by *every* grant it reaches — and, for `link`/`route`, by the node's transport author pin as well (§12.9) — and is not judged by `authors` at all; one reaching none is judged by `authors` alone. A policy may name either side without the other; one naming neither is refused at the boot. |
...
from the asking bundle's slot, not the key's: the link occupant's `link/sign` binds
`DOMAIN_link_scope ‖ network_key`, an ordinary app's `node/sign` binds
`DOMAIN_guest ‖ author ‖ app` (§12.2) — two separate seam names rather than one name with
two meanings, so gaining `link` adds the first without ever changing the second. Both
prefix and never parse, no op signs raw bytes, and the key itself never enters a realm —
...
`bootRuntime(opts)` (`host/main.ts`, `./shell`) assembles the pure-module builder, bundle-slot loader, capability backends, and node identity, then returns the `Shell` (`loadBundleBlob`, `invoke`, `dispatch`) and the channel adapter beside it. The CLI over it is `host/cli.ts`:
...
**Performance.** Because the Go target drives the *same* `libsodium.wasm` under wazero that the JS targets run under V8, crypto throughput tracks node closely — Ed25519 verify and XChaCha20 land within ~10% either way — and the Reed–Solomon codec runs a touch *faster* (measured Aug 2026 on a Ryzen 7 PRO 7840U: ≈1333 / 1751 MB/s encode / decode, vs node's ≈1159 / 1038 on the same machine with the same `codec.wasm`). The deliberate exceptions are the two native fast paths licensed by the rule above: the block-id hash (BLAKE2b-256, `golang.org/x/crypto/blake2b`), which sits on the storage data path and is the single primitive wazero ran materially slower than V8, so native (~1.1 GB/s vs node's ~940 MB/s) is the clear win; and the ChaCha20-Poly1305 record layer (RFC 8439, `golang.org/x/crypto/chacha20poly1305`), a per-frame cost that runs ~8× faster natively and, needing no scratch arena, takes no lock. Both are KAT-pinned against this build's own wasm output. Per-frame overhead trails node by Go-side event-loop cost, not crypto. Reproduce with `go test -run x -bench . -benchmem ./...` from `native/`; the node baselines come from seed store's `WASM/tests/bench.mjs`.
...
**The claim map is a projection of installed slots.** Every entry comes from a verified manifest's `protocols`; the shell recomputes it on each atomic slot replacement or removal. Nothing persists routing separately.
...
**Reserved ids, and the cross-realm call they carry.** Ids are held to a charset — alphanumeric first character, then alphanumerics and `._/-`, at most 64 bytes — and the leading-alphanumeric rule separates two spaces: an id spelled with a leading `_` is a local cross-realm service name. The spelling itself carries no authority and none is reserved to the runtime, so every `_`-led id claims under the ordinary rule of one active owner. What the leading `_` *does* decide is reach: a bundle's reserved claim is reachable by a co-resident guest and by no peer, which is a property of the routing rather than of the name — inbound delivery refuses it (below), and no sender's `requires` could have granted it in the first place.

Raw-link ownership and inbound routing are explicit authorities instead. Platform link events target the slot binding that owns `link/*`, without consulting the claim table. A guest granted `route/deliver` may submit `(claim, opaque attribution, payload)` to claim routing. The `route` grant is independent of `link`: holding sockets does not authorize invented attribution, and claiming a service name authorizes neither.
...
**A host may serve claims of its own,** through optional `bootShell({ claims })` — exact names, no wildcard and no fall-through, sharing one lookup and one conflict rule with the bundles' (a bundle contesting a platform name is refused as it would be against another bundle). The `_`-led rule binds the platform too: a reserved id is local-only whoever holds it, so a host name peers are meant to reach — seedchat's `offer/v1`, which carries a bundle between two browsers before either has an app that could receive it — is spelled as the ordinary id it is. Registering it is the deliberate act either way; what the spelling decides is reach, with no exception for host code. Attribution follows the path rather than the name: a peer key inbound, a caller app key from a co-resident guest.

**A guest reaches another realm by the same call, and only a reserved id is callable.** `host.call("_service", …)` resolves through this routing, the host prepends the CALLER's 32-byte id, and the answer is what the callee's `handle` returned on a later turn. A callable id is a *grant*, declared in `guest.requires`, so the local call graph remains explicit.

**Replacing the transport is ordinary slot replacement, not a protocol.** The same `(author, app)` loads a complete candidate offside, including its private modules, realm and scopes. After it stands and its freshness mark persists, the shell synchronously replaces that identity's slot, claims, and raw-link binding. The socket driver retains the address book and listeners, while the incoming guest reads immutable node identity and limits through `link/config`; after publication the driver replays its address book. Live links do not survive because their session keys were private to the outgoing realm, so an upgrade reconnects.
