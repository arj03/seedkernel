...
**Heap and deadline limits are per slot.** Each `loadBundleBlob` resolves its limit from that load's option, then the node default from `bootShell`, then the shared default. A replacement resolves its own limits rather than inheriting the outgoing realm's.
...
8. Re-check the host gates against the store as it now stands, persist the freshness mark for the version that successfully ran, then synchronously replace this identity — or the slot `replaceBundle` selected — in the installed slots and reproject every protocol claim directly to its slot. Only after that commit is the previous slot disposed. A candidate that a newer version has since marked past, one whose author was revoked mid-load, and one whose mark cannot be written are all discarded here, leaving the running version unchanged.
...
`loadBundleBlob(blob, options)` admits ordinary apps through `admit(v)`. It can update an ordinary app under the same `(author, app)`, but cannot load a bundle requiring `link` or implicitly replace the transport. An absent app predicate is `denyAll`. `authorAllowlist`, interactive consent, and `admitAll` express app trust; further checks, such as sensitive claim owners, compose with them as ordinary functions.
...
**Replacement selects the current owner explicitly.** `replaceBundle(oldAppKey, blob, options)` builds a candidate through the same installer and atomically replaces that slot, even across authors or app names. Ordinary app candidates still require app admission. A candidate requiring `link` is authorized only if the selected predecessor owns `link`; replacing an ordinary chat app cannot acquire raw sockets. Same-author transport updates also require this explicit operation.

The replacement may take the predecessor's claims but cannot take unrelated claims or overwrite another already-installed candidate identity. Only claims declared by the new manifest remain; claims it drops are released. The target is captured as an exact slot at call time and checked again at commit, so a concurrent update, replacement, uninstall, or shutdown makes the stale replacement fail. Construction, verification, admission, freshness, or persistence failure leaves the current owner running. Replacement works at the slot ceiling because it retires one slot as it installs one.
