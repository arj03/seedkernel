...
- **`MAX_OUTBOUND_QUEUE_BYTES` (16 MiB) and `MAX_OUTBOUND_QUEUE_SLICES` (4096)** bound
  ...
  queue on native): the driver keeps no model of that queue beside it, only the total it has
  admitted, handing back whatever the platform has since drained. Adapters do not decide
  admission, and one that cannot report fails its link rather than being read as holding
  nothing. `MAX_NODE_OUTBOUND_QUEUE_BYTES`/`_SLICES` (4× each) is the parent
