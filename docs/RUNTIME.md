...
3. **Initiation rate uses explicit scheduling.** A population bound is not a rate bound: a peer can replace each settled request with another. The timer table therefore paces the only fresh invocation roots a guest can create for itself. Externally initiated roots are bounded per invocation, with no aggregate CPU guarantee.
...
- **Self-initiated work against the node's clock** — a timer fire is the one **fresh invocation root** a guest can create for itself. A guest may make peerless cross-realm calls, but those are descendants of work already admitted and inherit its absolute deadline. `timer/arm(id, 0, …)` from inside the timer entrypoint is different: it schedules a successor with a fresh budget and could otherwise repeat forever without any external initiator. Each realm's timer table therefore meters the wall clock its fired invocations occupy — **one** period for however many are outstanding, since a serialized realm is occupied once — banks one whole invocation of that realm's budget, and earns the clock back at `1 / SELF_INITIATED_CLOCK_DIVISOR` (16, twice `DEFAULT_MAX_APP_SLOTS`). A deadline that comes due with the share spent is **slipped**, never failed and never dropped. The meter charges occupied time rather than the number of deadlines, so the transport's many cheap deadlines still fire promptly while a guest that spins fires progressively more slowly. A full node's summed self-initiated share is at most half its clock after the initial bank; `tests/verify-hardening.mjs` checks both that burst and the steady-state share. This is deliberately not a node CPU total: a peer can continuously replace settled work, so externally initiated work is bounded per invocation and not in aggregate.

  The meter observes the **wall** clock across a fire, not thread time inside it: the table hands the body to the realm and next sees its answer. A body parked on a host call is therefore charged as though it had spun, so this safe over-approximation may pace that realm harder but never less. The bundled transport's current timer callbacks return without awaiting the work they start, so they do not pay that difference.
