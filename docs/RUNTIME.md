...
- **`MAX_OUTBOUND_QUEUE_BYTES` (16 MiB) and `MAX_OUTBOUND_QUEUE_SLICES` (4096)** bound
  ...
  "our queue" and "the platform's". The adapter reports what it still holds (`RawLink.buffered`
  — `writableLength` on Node, `bufferedAmount` in the browser, an exact queue on native) and
  the driver reconciles retained bytes against the delta of that report; adapters do not
  decide admission, and one that cannot report fails its link rather than being read as
