...
**The assembly order is shared too, not just the parts.** `createShell` (`host/shell-core.ts`) owns verification, admission, module construction, scope derivation, realm construction, freshness, atomic claim replacement, and inbound dispatch. Each target supplies only `{ sodium, identity, modules, fs, freshnessStore, networkKey, transportHost, createRealm }`. `transportHost` remains platform integration state rather than a `Shell` member; the shell attaches it directly to the current `_net` slot. A guest that fails to stand or a freshness mark that fails to persist disposes only the candidate, leaving the running slot intact. Go therefore holds no boot sequence, admission rules, routing model, or operator flow of its own.
