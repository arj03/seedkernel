...
| `grants` | at least one of `authors` / `grants` | Who may hold each **privilege**, keyed on the privilege itself. The keys are the host's `PRIVILEGES` (derived from `AUTHORITY_CALLS`, `core/domains.ts`); a key that names none is refused at the boot, so an operator cannot misspell a grant into a node that holds nothing. Today there are two, both deliberate decisions apart from `authors` and never app code: `link`, the raw links the node's transport is built out of — the holder carries the node's whole network and signs the channel AUTH (§12.2) — and `route`, submitting an inbound request whose *attribution the submitter writes*, which is why it is not folded into `link`: holding sockets is not the same as being trusted to say which peer a request came from. A bundle reaching a privilege must be admitted by *every* grant it reaches and is not judged by `authors` at all; one reaching none is judged by `authors` alone. A policy may name either side without the other; one naming neither is refused at the boot. |
