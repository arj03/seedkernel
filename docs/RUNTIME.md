...
| `link/config` | (empty) | static transport configuration: `[identity blob][network key blob][contact secret blob][limits u32 …][admitted peers blob]`. Immutable node facts only — the mutable address book is delivered separately, as `addr` events after `_net` is published. The shape is versioned by `guest.abi`, so removing or reordering a field bumps it and appending one does not (§12.4). |
...
**Every pure question about a bundle is a gate, and each gate sits where its question is decided.** Who may claim a reserved id is a fact about the manifest — the privilege is derived from the same signed `guest.requires` — so it is checked at verify with the other well-formedness rules (`verifyManifest`, bundle.ts) rather than at admission: `_host` is claimable by nobody, and `_net` only by a bundle that reaches `link`. The implication runs one way, and only that way: `link` does not imply `_net`, since a link-capable bundle claiming nothing is an ordinary initiator. The partial-claim gate is gone with the thing it guarded — a privilege is one thing, so nothing reaches the unprivileged base in half. Admission's own two (`notRevoked`, `freshVersion`) are the `Admit`s in `hostGates`, composed by the shell rather than run next to its `admit(…)` call. A rule that refuses a bundle from outside the predicate would be a second decision point, and a second decision point is where "nothing has landed until the predicate says yes" stops being a property of the type and becomes a property of how carefully the load path is read.
...
**Reserved ids, and the cross-realm call they carry.** Ids are held to a charset — alphanumeric first character, then alphanumerics and `._/-`, at most 64 bytes — and the leading-alphanumeric rule separates two spaces: an id spelled with a leading `_` is a local cross-realm service name, one no peer can reach and only a co-resident guest can call. The spelling itself carries no authority, so an ordinary `_`-led id claims under the ordinary rule of one active owner; the two the runtime keeps for itself, `_host` and `_net`, are named at verify rather than derived from the leading character. That keeps the charset a property of the format instead of a list every target must remember to check, and the two exceptions short enough to state in a sentence.

Two names in it are the runtime's, and each is refused to a claimant for its own reason. The **shell** answers `_host` itself, ahead of `dispatch` — the way the transport reports an inbound request and the fate of a host-managed link — so nobody claims it. It is answered for the realm currently claiming `_net` and no other, which is a tighter test than the `link` privilege on purpose: its `deliver` op hands the routing a frame with a `from` the caller writes, so anything reaching it could forge an inbound request attributed to any peer. Being the node's network earns that; holding `link` and claiming nothing does not. The **transport** claims `_net`, the service name apps call, and only a bundle reaching `link` may: that id is where every accepted link's raw bytes are handed in, before any peer has authenticated, so an ordinary app serving it would read the node's unauthenticated traffic without ever being granted the privilege that governs exactly that — and, since a claim has one active owner, would squat the id the real transport needs. The claim still grants nothing; `link/*` and policy remain what hand out sockets.

**A host may serve reserved ids of its own,** through optional `createShell({ answer })`, consulted on inbound frames before claims. `null` falls through to the routing table for an ordinary wire id, and to nothing for a reserved one — which is where "no peer can reach one" above is actually enforced, since a bundle's reserved claim is a local service name no `requires` of the sender's could have granted. The hook is inbound-only; a co-resident guest calls a reserved claim and is attributed by a host-derived caller id.
...
**Replacing the transport is ordinary slot replacement, not a protocol.** The same `(author, app)` loads a complete candidate offside, including its private modules, realm and scopes. After it stands and its freshness mark persists, the shell synchronously replaces that identity's slot and claims. A different identity contesting `_net` is rejected rather than shadowing it. There is no handover: the socket driver retains the address book and listeners, while the incoming guest reads immutable node identity and limits through its side-effect-free `link/config` capability. Once `_net` is published, the driver replays its current address book through the ordinary `addr` event before any following request can overtake it — same listening port, same node identity, with no mutable snapshot taken offside. Live links do not survive because their session keys were private to the outgoing realm, so an upgrade reconnects.
