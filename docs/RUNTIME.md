...
| `timer/arm` | `[id u32][ms u32]` | (empty) - re-enters `handle` with the TIMER caller id |
...
**The node's immutable facts are an argument, not a name.** The host invokes the freshly stood link occupant's `handle` once - before the binding is published, with the host's caller id - carrying `[identity blob][network key blob][contact secret blob][limits u32 ...][admitted peers blob]` behind the driver's own framing of the ``init`` event (host/transport-host.ts `initialConfig`, a contract with the pinned bundle, not a kernel ABI). It is the constructor's input: only events and raw `link/*` bytes reach the occupant afterwards, and nothing about it is re-readable. The address book is mutable node state and is not among the facts: it arrives as `addr` events, replayed after publication (12.10). The shape is versioned by `guest.abi`, so removing or reordering a field bumps it and appending one does not (12.4).
...
| Name | Serves | |
| --- | --- | --- |
| `node/sign`, `node/verify`, `node/identity`, `node/random` | the node key, scoped signing/verification under THE slot's one scope — an app's own, the link slot's network scope — and the entropy source — what the *host* owns | |
...
| `link/open`, `link/send`, `link/close`, `link/stat`, `link/authenticated`, `link/down` | **`link` privilege only** — bytes over an opaque link id, the platform's whole contribution to the network, the reports back to the binding that supplied one. The link slot's scoped signing/verification is `node/sign`/`node/verify` under the slot's network scope (§12.2) | |
...
| `guest.abi` | integer | **yes** | Which host seam and preamble this guest was written against (`GUEST_ABI_VERSION`, 12.2). A guest declaring an ABI this host does not implement is refused by name - a legibility failure, like an unsupported signature suite, not an authenticity verdict. Required rather than defaulted: the default would have to be the oldest ABI, which is exactly the population a bump exists to catch, and a guest author who never considered the seam version is indistinguishable from one who meant the old one. The number tracks changes to the seam's *shape* - the naming scheme of `host.call`'s first argument, a name moving across the sync/async line, the entrypoint protocol (`handle([caller 32][body ...])`, one entrypoint, the `__deferred` marker), or a preamble global changing meaning - not the appending of new names, which a guest that never calls them cannot notice, and NOT the callee's own format after the caller id, which is content. ABI 12 is that shape: the op envelope left the preamble for good. Its reason for existing is that the failure it guards is silent: a guest calling `host.call("fs/get", k)` without `await` gets a Promise where bytes were expected and reads `undefined` - a wrong answer, not an error, and one no care at the call site turns into a loud one. A declared seam version makes it a refused load. |
...
A serving node that has loaded a bundle serves the app's *request* side from its confined realm: an inbound frame reaches the guest's **one** entrypoint, `handle`, with the authenticated sender prepended (`dispatch` -> `realm.call`), and the host drives the app's local logic through the **same** `handle` by a loopback - `invoke(payload)` writes the host's own caller id (32 zero bytes) in front of bytes it never reads, so a peer frame and a host loopback both arrive as `handle([caller 32][body ...])`. An app therefore has one op vocabulary rather than two, and it is the app's: everything after the caller is the CALLEE's format, and the kernel's whole inbound vocabulary is that 32-byte attribution prefix. A guest declares `handle` - nothing else is ever invoked. The op name a CLI or an operator script needs travels in the payload, framed by the caller with the app's convention (the CLI composes `[opLen u8][op][args]` itself, host/cli.ts `opInput` - the contract is between the operator and that one app, never the kernel). Both directions may `await`; the realm serializes them (12.3), and the driver resumes on the promise the app's `handle` returned rather than answering inline - which is what lets an app's inbound handling be asynchronous at all. The shell is application-neutral - it can host any signed app - and for a self-contained non-browser deployment the Go/native target ships it as a single binary (12.9). seed store's WASM README has a complete storage walkthrough.
