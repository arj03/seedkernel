...
*The runtime as an app host: performance, the chat demo, and the host — host services, the guest-seam ABI, zero-authority JS realms, signed bundles, the node↔node transport, and the Go/native binary.*
...
- **Per bundle load:** one BLAKE2b-256 hash of the complete body and verification of both Ed25519 and ML-DSA-65 signatures (§12.4). Once, at install: the bundle envelope is never on the message path (§13).

The Go/native target carries `*_bench_test.go` benchmarks over these hot paths (net round-trip, fs, the crypto primitives, the record layer); `npm test` from `WASM/` exercises the same paths end-to-end on the JS target, and seed store's `WASM/tests/bench.mjs` measures storage throughput.
...
| QuickJS realm engine (the in-repo quickjs-ng 0.16.2 emscripten build, `quickjs/` — the same engine source the native binary compiles, §12.9) — only loaded when a bundle's guest runs (§12.3) | ~570 KB |

The `host/*.js` layer is the whole runtime: bundle slots and claim routing, raw net and fs seams, the guest seam, safe-js, verification, policy, and the transport driver. Pure modules remain target values owned privately by a slot. The transport protocol is signed content (`transport/src/*.js` plus `ws.wasm` and `mlkem768.wasm`); the first copy ships inside the artifact because a node has no network until it has a transport, and a later signed bundle can replace it. Core libsodium backs the trust-root crypto and the current channel's remaining host transforms. QuickJS is lazy: a host with no installed slot never pays for a realm.

`npm run build` emits the host twice: the readable `build/` (~482 KB of runtime code, doc comments intact) for debugging and a comment-stripped `build-min/` (~301 KB in the current build) for shipping. `build-min/` holds only what the browser entry points listed in `tsconfig.min.json` reach, so the Node-only, native-only and bundle-authoring modules never ship to a page. More than half of the minified tree is the generated `transport-bundle.js`, the signed bundle embedded verbatim — it carries no doc comments to strip, so it passes through nearly unchanged and dilutes the ratio. The hand-written browser host alone goes ~239 KB → ~124 KB, because those sources are close to half doc comment, and that is where the cut comes from. It is a second `tsc` pass with `removeComments` (`scripts/minify.mjs` over `tsconfig.min.json`) — no bundler and no new dependency, and the compiler is the one thing on hand that can tell a regex literal from a division, so nothing hand-written is lexing the host; it prints the current plain and gzipped totals on every run. The table separates the hand-written host from the embedded transport so it does not count either twice.

---

## 11. Example app layer: chat ([seedchat](https://github.com/arj03/seedchat))

Chat is the smallest possible app: a confined JS **guest** over a single **restartable** module (§4). The guest is a handful of lines — its `handle` entrypoint forwards its input to the module by name through `host.call` and returns the render bytes — and the module is the transform: it does no I/O and no crypto, reading `senderPk ‖ chatType ‖ body` and writing the render bytes for the UI. Everything around it — authenticating the sender, moving frames, driving the iframe — is the host's and the chat page's job, because a module has no reach of its own and a guest reaches the world only through `host.call` (§12.2).
...
What the demo stands up is a browser page owning the host's table and its one install path, a WebRTC socket seam (`RtcNetwork`, `services/net-rtc.ts`, §12.7) under the transport bundle, the safe-js guest realm, and a sandboxed iframe — every byte of chat UI and logic arrives as a signed bundle admitted at runtime.

On load it generates an Ed25519 channel identity, constructs a host (§3), and loads an admission policy (§12.5) approving app authors the user trusts. Seedchat's authoring tools build each **hybrid-signed bundle** with Ed25519 and ML-DSA-65 public keys and signatures (both required) over one body containing the manifest, guest and WASM modules (§12.4). Seedchat derives its consent key by hashing the verified WASM bytes it already holds. The table starts empty. A user may install `v1 — text only` or `v2 — text + image + nick`; a v1→v2 release under the same `(author, app)` is an install naming that slot, landing atomically and re-stating the `chat` protocol claim. A different app that contests an occupied claim is refused rather than silently taking it (§12.10). Peers may relay these bundles in an `OFFER` frame; the recipient re-verifies both original author signatures and applies its own policy, so the bundle remains attributable across any number of relays.

Peers connect over a WebRTC mesh from `RtcNetwork` (`services/net-rtc.ts`, §12.7). Every delivered frame is already attributed to an authenticated peer, so chat rides the Transport request plane as `[req][protocolId][type][chatType‖body]`. The host resolves the protocol claim directly to a slot, prepends that peer key, and invokes the slot's guest under its budget; the guest may call its private chat module. A peer states only the protocol, while the receiving user's installed claims decide which verified implementation renders it.
...
Chat (§11) is a browser page wired by hand, in its own repo. The same onion ships as a **general runtime artifact** — the host — that any app rides on as **signed content**. The host knows nothing about chat or storage; it offers a fixed, generic surface, verifies a bundle against a policy, and *becomes* whatever the bundle is. [seed store](https://github.com/arj03/seedstore) is the worked example: a full peer-to-peer storage node is the host plus a signed bundle, with no storage-specific code in the runtime.

The manifest's `guest.requires` is **everything a guest reaches** outside itself (§12.2, §12.4): the host services it is granted — `node`, `fs`, `timer`, `link` — and the local service ids it calls on a co-resident guest (§12.10). `link` requires explicit transport selection (§12.5). The list answers "may this *app's guest* reach this backend or that realm at all?" (WASM modules, by contrast, hold no grants at all — a module reaches nothing but the input it is handed, its own private memory, and the output it returns, §4.2.)

**Only reach is declared.** A transform is computation, not authority, and belongs in the bundle that uses it. A bundle's modules are installed and verified with its guest, so calling one reaches nothing the guest does not already hold — the scope (one app's map) is the shape, not a grant. The residual `crypto/*` names — host entropy included — are a compatibility table for transforms the host already carries; they are ungated and closed to new algorithms. Time is not a grant either: every realm reads `Date.now()` and `performance.now()` as ordinary ECMAScript intrinsics. So `requires` carries exactly what a guest could not otherwise touch — the node key, disk, its wake, sockets, and other realms — and the list an operator reads is the bundle's whole reach.

The field lives inside the signed manifest and nowhere else; the resulting backends and scopes are captured by the bundle slot. **`protocols`/`services` are what a bundle serves, to peers and to locals; `requires` is what it reaches, on the host and on locals.** Which half of `requires` a name belongs to is one closed-table lookup (`isService`), never a spelling convention.

### 12.1 Host services: raw-byte backends

The host provides the backends behind its host services (`node`, `fs`, `timer`, `link`) and a small table of host transforms, all driven by an app's confined logic through the guest seam (§12.2). They are deliberately structureless — bytes in, bytes out — so the host never learns what an app means by them:

- **Host transforms and bundle modules.** `HOST_TRANSFORM_NAMES` is the residual `crypto/*` compatibility table: `blake2b-256`, `chacha20poly1305-ietf/{seal,open}`, `x25519/dh`, and `random` — the host's entropy source, ungated because random bytes reach nothing, so a guest needing a nonce is never handed the node key to get one. It is not a grant and is not a catalog to provision ahead of callers, and it is at its floor rather than mid-removal: each name is one the host's trusted base already carries and calls itself, so the name costs nothing the artifact does not already contain, while dropping it would push a duplicate — or, for `x25519/dh`, an unvendored and handshake-critical — implementation into every guest (SECURITY §14). New pure computation ships under a bare module name in the bundle that needs it. The default transport demonstrates the rule: `host.call("mlkem", bytes)` reaches its private `mlkem768.wasm`. Seed store ships its erasure coding the same way, under its own `codec` module name; its hashing and record encryption still reach the residual table, while its signatures use the scoped `node/sign`/`node/verify` pair.
- **The authorities** — everything that reaches something no confined module can hold. `node/sign` under the node identity but **scoped**: the host prepends a domain and a host-derived scope to the message before signing (§12.2), so a guest never obtains a raw node-key signature and raw signing stays host-internal. `node/verify` is scoped the same way — the host applies `domain ‖ scope` to a caller-named key's signature, so a guest checks a signature under its own bundle's namespace and never reconstructs the prefix. `timer/*` (the platform's event loop). The node's public key is not a call: every realm reads it as `HOST.identity` (§12.4).
- **`fs`** — raw bytes under an opaque, flat key (`services/fs.ts`): `get`/`put`/`size`/`list`/`delete`/`stat` (existence is `size ≥ 0`, so there is no separate `has`). An in-RAM `MemoryFs` (`services/fs-memory.ts`) and a directory-backed `NodeFs` (`services/fs-node.ts`), with OPFS/IndexedDB the shape a browser backend fills in. No content-addressing, no paths — that's app policy. `MemoryFs` carries its own byte and entry quota (`DEFAULT_MEMORY_FS_MAX_BYTES`, `DEFAULT_MEMORY_FS_MAX_ENTRIES`), because a host call bounds requests *in flight* while a successful `fs/put` retains bytes after the call settles: without a quota the in-memory backend turns bounded input into unbounded permanent process memory. A disk backend takes the medium's own limit instead.

  **Every method is asynchronous, and that is the seam's property rather than any backend's.** A synchronous `get(key): Uint8Array | null` is a shape no browser backend can implement — IndexedDB is asynchronous by construction and OPFS is synchronous only inside a Worker — so a sync seam would have made the browser the one target unable to carry a host service (§1). `MemoryFs` and the native target's Go primitive both answer in the call and are wrapped to resolve in a microtask, because a seam that resolved sometimes-immediately would let a guest work by accident on one backend and fail on the backend it ships against. It follows on the guest side too: the `fs/*` names round-trip like a cross-realm call, so a guest reads them with `await`.
- **Raw links, structured service, and inbound delivery.** `link` moves bytes over an opaque link id and binds platform link events directly to the slot holding that service — one slot, since the driver has one event sink (§12.10). The transport exposes its structured API under an ordinary local service id selected by composition (`_net` in the bundled setup) — declared in the manifest's `services` list, a co-resident guest's to reach and never a peer's. Inbound delivery is `link/deliver`: the occupant that sees plaintext hands the host `(claim, attribution, payload)` to route and gets the answer back to frame, so the inbound path mirrors the outbound one. It confers nothing extra, because it names no link and the caller already chose all three fields. The driver's three events and the one-byte reason `linkClosed` returns are specified with the names, in §12.2.

  **A `RawLink` states only what the guest cannot know.** Its `stream` bit distinguishes message-preserving transports from byte streams; it carries no codec or authority. The guest derives those from its own dial destination. For platform-opened links, a separate `Arrival` supplies the listener label and, for a link the platform dialed on the node's behalf, `dialed` — the peer it was dialed for. WebRTC sets it on the side signaling chose to initiate. Guest-opened links have no `Arrival`. Thus `link/open` returns only a link id and `stream`, while the host still decides whether it can route the opaque destination and retains the descriptor.

  Under the platform the socket seams are `services/net-node.ts` (node:net), `services/net-rtc.ts` (WebRTC, §12.7) and `services/net-ws.ts` (a browser `WebSocket`), each handing a link to the driver in `host/transport-host.ts`; the flood bounds that must sit with whoever holds the descriptor are `services/net-limits.ts` (§12.6.2), declared by the host and applied by the bundle's framers, which are the code that sees a length before its body.
...
Anything with *structure* is a **pure module** that transforms bytes: WebSocket framing is `ws.wasm`, a module of the transport bundle; Reed–Solomon erasure coding is an app's `codec.wasm`. Both are pure modules their own bundle's guest drives by logical name, never something the host or the platform knows.

### 12.2 The guest seam: the guest name ABI

An app's confined logic reaches all of the above through a single seam, `host.call(name, bytes)` — the guest's one route to real I/O. `host/guest-seam.ts` (`./guest-seam`) services that seam from the primitives above and *only* those. Every name is application-neutral; the seam has no idea it is hosting storage.
...
**A name is whatever the manifest declared it as.** A name in THIS realm's `guest.requires` that is not a host service is another realm's local service; a name in its `modules` is its own module; every other name is looked up in the host table (host services and residual host transforms). The seam builds one map of the declared names at load and falls through to the host table, so dispatch never reads a name's spelling. Install keeps the three disjoint: a local service id may not live in a host namespace (`fs/…`, `crypto/…`) or spell one of the bundle's module names, and a module name cannot contain the `/` every host name carries (`[A-Za-z0-9_-]`, §12.4). `crypto/*` is generated from `HOST_TRANSFORM_NAMES`; `HOST_SERVICES` is hand-written. There is no second framing: the module name is the seam's name argument.

**Two kinds are not grants.** The residual `crypto/*` entries and a bundle's own bare module names are ungated because they are pure computation. Every other host name belongs to a service touching the node key, entropy, clock, sockets or disk. New algorithms go in the bundle-module half, not `HOST_TRANSFORM_NAMES`.

| Name | Request | Response |
| --- | --- | --- |
| `crypto/<host-transform>` | argument bytes | output from the frozen compatibility table (§12.1); an unknown name throws |
| `crypto/random` | `[n u32]` | `n` bytes of host entropy — ungated |
| `node/sign` | message bytes | 64-byte detached Ed25519 signature under the node identity, over `domain ‖ scope ‖ msg` — this slot's one scope (`slotSignScope`, below, §16.1): an app slot's own `app` label, an empty scope after `DOMAIN_link_scope` for the link slot. The host chooses what the name signs under; never guest-supplied |
...
| `timer/arm` | `[ms u32][tag 4]` | (empty) — replaces this realm's one armed wake, due in 0..2147483647 ms. It arrives as `handle([caller 32 zero bytes][tag 4])`, the tag opaque guest content; a realm that has spent its share of the node clock has it slipped rather than failed (§12.3) |
| `timer/clear` | (empty) | (empty) — cancels the armed wake; a notification already handed to the realm is not retracted |
| *any name in this realm's declared local services* | opaque bytes, with the CALLER's 32-byte id prepended host-side | whatever the claiming realm's `handle` returned — **awaited**. A local service id (§12.10) with no host-known spellings or roles; refused by name when nothing claims it |

The `link` names are the **raw-link** service. Raw events arrive directly at the one slot holding it; what a transport provides back is an ordinary local service claim. `linkOpen(linkId, …)`, `linkBytes(linkId, bytes)` and `linkClosed(linkId)` are the whole of it, and they are a declared ABI rather than a convention: they are the `link` service's `events` list in `services/domains.ts`, beside its `calls`, so a service says both what it is called with and what it calls back with, and an author replacing the transport reads exactly one table. The first two events are one-way — who a link reached and whether it authenticated are the occupant's own state, and the host neither asks nor keeps a second copy. `linkClosed` alone answers: **one byte, the reason** (`transport/src/ake.js` `REASON_*`). The shipped vocabulary splits on the question an operator actually has. Before authentication: `dropped` — the socket went away under an unfinished handshake, a refused connect, an unreachable host or a far end hanging up, which is the ordinary shape of "the other machine is not there" and the line a dial at a dead address really prints; `refused` — the peer provoked the teardown, a bad contact-secret probe, a malformed or over-cap handshake frame, a signature that did not verify, an identity the admit lint rejects; `timeout` — our deadline fired on a socket that stayed open and silent, a firewall swallowing packets or a peer that stalled; `handshake` — what is left, which is ours rather than theirs (a half-open budget evicting us, a local failure). After it: `clean`, the peer's own end-of-stream record; `aborted`, a teardown a peer provoked; `local`, our own deliberate shutdown; `truncated`, a stream that just stopped — the post-auth form of `dropped`. That is the reason this is a **return** rather than a guest-callable name — a name would let the occupant say it about a link it did not observe it on — and it carries no link id, so it cannot redirect a transition to another socket.

The byte is a **local** fact that never goes on the wire, which is what lets the pre-auth cases be told apart without weakening the concealment of §12.6.2: a refused caller still observes silence and then a socket close, identical to every other refusal and to a port that is not listening. The driver prints the reasons that are not routine — everything but `clean` and `local` — as `[transport] link N from <addr> down: <reason>`, so a node whose links are failing says so without an embedder wiring anything (`TransportHost.suppressLinkLog` silences it for a relay). That printing is the only place the driver reads the vocabulary, and nothing in it branches on a code: a replacement transport with its own numbering is not wrong, its codes print as numbers.

**Raw-link event byte layout.** The socket driver encodes these events with `OpArgs` from `services/op-frame.ts`, and the host prepends its own caller id. Each event arrives at the link occupant's `handle` as `[caller 32 zero bytes][opLen u8][op ASCII, opLen bytes][args …]`. The envelope and the following field order are host ABI: every replacement link occupant must accept them. There is no padding or terminator. Every `u32` is unsigned big-endian; `blob` is `[byteLength u32][bytes …]`, and `text` is a blob containing UTF-8 bytes (the length counts bytes). Boolean fields are one byte, `0` or `1`.

| Event | `opLen` | `args`, in byte order | Return body |
| --- | --- | --- | --- |
| `linkOpen` | 8 | `[linkId u32][stream u8][listener text][dialed blob][remoteAddr text]` | Ignored by the driver |
| `linkBytes` | 9 | `[linkId u32][bytes blob]` | Ignored; the driver awaits completion before admitting the next read on this link |
| `linkClosed` | 10 | `[linkId u32]` | `[reason u8]`, with no envelope or caller prefix; an absent, malformed or rejected answer is reported as `0` |

`linkId` identifies the host-owned socket; `0` is never a live link id. `linkOpen` announces a platform-accepted or platform-initiated socket: how it arrived, plus the one property of the socket itself (`stream`). A socket opened by the guest through `link/open` gets its id and stream bit in that call's return, without a `linkOpen` event. Missing arrival metadata encodes as an empty listener and an empty `dialed` blob, which is an accepted socket; a dialed peer id is its 32 decoded key bytes, and makes this end the handshake's initiator. A missing source address is empty text. The listener and source address are platform metadata, not authenticated peer identity. `stream = 1` makes `linkBytes` an arbitrary byte-stream slice; `0` makes it one platform-framed message. The host constructs this event framing while leaving the bytes inside the `linkBytes` blob opaque. The codec remains optional for application and ordinary local-service bodies, whose op vocabularies belong to their callees.

**Transport configuration belongs to the transport.** The host passes each load's installation-local `LOCAL` object unchanged, including for the link occupant. The shipped transport reads `networkKey` and `contactSecret` from it as 64 lowercase hex characters, rejects malformed explicit values during load, and defaults each absent value to 32 zero bytes (the public network and an open contact gate, respectively). The node's own **identity** is not configuration: the occupant reads it from `HOST.identity`, which the host fills from the keypair `node/sign` signs with, so the key it publishes and the key its transcripts are signed under cannot disagree. The address book also belongs to the occupant: peers arrive through the embedder's ordinary `transport.config` and through `addr` calls afterwards (§12.10).
...
| `link` (`link/open`, `link/send`, `link/close`, `link/deliver`) | **The link owner only** (boot selection or explicit replacement, §12.5) — bytes over an opaque link id, the platform's whole contribution to the network, plus the attributed inbound request that comes back the other way. Authentication and teardown observations are link-scoped, so they return from the driver events that caused them rather than travelling through guest-callable names. The link slot's scoped signing/verification is `node/sign`/`node/verify` under the constant `DOMAIN_link_scope` (§12.2) | |
...
**The `link` service selects transport admission.** A manifest naming it is accepted only as the selected boot transport or an explicit replacement of the current link owner (§12.5). Adding `link` cannot turn app trust into transport trust. The service covers raw sockets and attributed delivery; `timer` remains an ordinary service.

Calling the transport's local service does not acquire raw-link authority. An app declares that service id in `guest.requires` beside any host services and calls it through the ordinary local-call path.

A name the manifest never granted does not resolve — the seam refuses it, and the host never wired the backing resource in the first place (an fs-less bundle gets no fs backend at all, not an fs backend behind a check). An unknown SERVICE in a manifest throws at load — the vocabulary is closed (`HOST_SERVICES`, `services/domains.ts`), so a typo fails loudly rather than silently granting nothing, or, worse, everything.

**Relation to WASI.** The guest seam is deliberately WASI-shaped at the seam: a small syscall table, a zero-authority guest, and an ungranted service left unwired rather than checked at runtime. The differences justify a bespoke ABI. The names are not POSIX — `link` moves bytes over opaque link ids with no socket API, and peers are addressed by pubkey inside the transport bundle rather than by the host (§12.6); `fs` is a flat opaque blob store with no paths; `node/sign` and `HOST.identity` surface the node's identity, which WASI has no notion of (and every guest signature is domain-scoped). And the grant is *signed content*: the guest's authority is the `guest.requires` field of an author-signed manifest (§12.4) admitted by operator policy (§12.5), where WASI's grants are host-local instantiation choices with no authorship. WASI begins after who authored the code and who may install it are settled; §12.4–§12.5 settle them. What keeps this from drifting into a worse WASI: names stay structureless bytes, anything with structure becomes a pure module (§12.1), and the catalog grows by adding names sparingly.

**`fs` is scoped to the app, not the node.** The backend a guest reaches is wrapped per **app label** (§12.4), so `fs/list` with an empty prefix enumerates that app's keys and no others, and `fs/get`/`fs/delete` cannot name another app's. Keys come back stripped of the scope, so the guest only ever handles the names it chose. The scope belongs to the label, not to the slot or its author: uninstalling leaves the keys where they are, and whatever installs the label next — a new version, a fork, an author's rotated key — reads them. The scope prefix is a hash of the label (`appScopeFor`), not the label itself: keys double as **filenames** and both backends restrict them to `[A-Za-z0-9._-]`, which an author-chosen `app` cannot be trusted to stay inside, and a case-folding filesystem would merge two labels differing only in case. Hashing to lowercase hex fixes both at once, and its fixed length means two distinct labels cannot produce prefixes where one extends the other (plain concatenation would let app `x` key `yz` collide with app `xy` key `z`). The charset is a consensus predicate (`isSafeFsKey`, §16.1): `.` and `..` are refused, and so are Windows device names (`CON`, `PRN`, `AUX`, `NUL`, `COM0`–`COM9`, `LPT0`–`LPT9`) on every OS, because Windows resolves the stem before the first `.` to a device (`NUL.txt` is still `NUL`) and two nodes disagreeing about a key disagree about their contents. `fs/stat` is deliberately **not** scoped: `used`/`available` describe the physical backend, and a per-app `available` would be a fiction.
...
**And one way in.** `realm.call(bytes)` — one invocation of the guest's single `handle` entrypoint — is the whole seam, for the initiator and the holder alike. There is no entrypoint *name* on the seam to pick: the argument is `[caller 32][body …]`, of which only the caller id is the host's, so which role an invocation serves is something the guest reads out of its own body format and never something the host selects. A synchronous second seam is not available to be offered either: a holder answers from local storage, and storage cannot answer in the same turn on a target whose backend is asynchronous (§12.1). Both roles are therefore ordinary async invocations of that one entrypoint, and there is one shape to reason about rather than two whose difference is a property of what the guest happens to call.
...
This is the chat page's sandboxed-iframe confinement (§11) generalised: "run zero-authority guest JS over a cap seam," the sibling of "run a WASM module under caps."
...
2. **Causal lifetime uses a monotone deadline.** One absolute deadline begins at an invocation root and can only shrink through queues and calls. It bounds one causal chain; no callee can renew it. The link occupant is the one exception, below: its turns are always its own.
3. **Initiation rate uses explicit scheduling.** A population bound is not a rate bound. The realm wake therefore paces the only fresh invocation roots a guest can create for itself. Network-originated application roots arrive only over mutually authenticated links; that gives the replaceable transport a stable peer identity on which to impose admission and fair-scheduling policy. Authentication is attribution, not trust — an authenticated peer may still be hostile — and the current transport does not aggregate its roots. Externally initiated roots are therefore bounded per invocation, with no node-wide CPU guarantee.

Custody itself admits no exemption flag and no operation name relaxes it — a name may *tighten* what an owner admits (`fs/put` meets a storage quota), and whoever owns the resource enforces the bound rather than the dispatcher that routed the call. Each owner is finite, has a complete teardown path, and is bounded in time as well as size: a release path nothing is committed to calling bounds only until the freeing event fails to happen. The owners below each default to a real number, so a host that configures none still gets a bounded guest:
...
- **A bound on realms** — every owner above is per realm, and a node holds many, so what the node holds is one realm's ceiling times `DEFAULT_MAX_APP_SLOTS` (refused at `install`; a replacement retires a slot as it installs one, an uninstall gives one back). Bounding the multiplicand rather than pooling the products is what makes those per-realm numbers ceilings instead of floors — and it is also the better isolation. Apps in one node do not trust each other, and an allowance they draw on in common is a standing way for a busy app to refuse a quiet one's calls; a quota each times a bound on how many there are reaches the same total with no such channel. Where the tenant is one, a shared pool is the right shape and the driver uses one: every socket belongs to the single link slot, so `MAX_NODE_OUTBOUND_QUEUE_BYTES` pools across links (§12.6). **Pool within a tenant, quota between them.**

**Disposal fails what is parked.** `Shell.close()`, uninstall and revoke dispose a slot's realm at once. Disposal rejects every invocation still parked in the realm — a repair pass waiting out an unreachable peer included — and frees the engine on a later turn, so no call resumes into a freed QuickJS context. A handle held past that rejects by naming the slot rather than calling into a freed realm.

The first two cross every seam between the operator and the realm — CLI flag, `bootNodeShell()`, `bootShell`, `InstallOptions`, `RealmFactory` — because a bound the host accepts but no target can set is a bound nobody has — one the realm factory takes and nothing upstream carries is dead, since the default applies and nothing can change it. `--guest-timeout 0` reads as "no budget", so disabling one is something an operator says rather than something a missing or mistyped flag does: a value that is not a whole number is refused, and `bootShell` refuses a heap limit below 1 byte or from 2³² up, or a budget under 1 ms, since the two engines would read those differently (the JS engine takes a heap limit of 0 as none).

**Heap and deadline limits are per slot.** Each `install` resolves its limit from that call's option, then the node default from `bootShell`, then the shared default. A replacement resolves its own limits rather than inheriting the outgoing realm's.
...
```
manifest            the app's label, claims and guest authority
guest               the app's zero-authority guest program (§12.3)
modules             WASM bytes, in manifest order
```

A bundle is a **value, not a path**: one blob is read from disk, carried in an `OFFER` over a data channel, and stashed in browser storage without a second format or a second load path. Both signatures cover the entire body, including its lengths and module order. The verifier authenticates the body before interpreting any of it.

**There are no bundle filenames.** The body carries length-prefixed manifest JSON, guest UTF-8 source, then one WASM byte string per manifest module, in that order. Every length is an unsigned 32-bit big-endian byte count. The manifest's module list determines the number and names of the modules. Missing fields or trailing bytes reject the bundle.

**What a bundle carries.** A load constructs one slot from everything an app is made of:

- **The guest is the app; the modules are its library.** The guest is JS source for a QuickJS realm, not WASM — the install path ends in "instantiate WASM, bind," and the guest is what the host runs. Modules are the restartable transforms the guest drives by name; a bundle may declare zero or many of them. Without the manifest the guest would have no signed identity at all.
- **The guest's authority has no other home.** A WASM module is a transform with no grants of its own (§4.2); the guest, by contrast, *does* reach I/O — through the guest seam — yet is not a module and has no table entry of its own, so the manifest's `guest.requires` is its entire grant.
...
- **Operator/author separation.** The host is one fixed, auditable artifact; the app arrives as content signed by a third-party key the operator's policy admits. Verification is channel-independent: a bundle read from a USB stick verifies exactly like one fetched from a mirror or pushed over a relay.

**One authenticated statement, one authorization.** The signatures commit to the manifest, guest and every module; `verifyBundle` proves the bytes, and one policy decision authorizes the whole set. The host then constructs a complete slot offside and commits its claims atomically (§3.1). A live update is the same path with a higher version, naming the slot it retires so that what dies is a thing the operator chose rather than a coincidence of keys.

**Bundle envelope.** One suite, `0x02` — hybrid Ed25519 + ML-DSA-65. A fixed-width header followed by the signed body:
...
body = [manifest_len: u32][manifest JSON][guest_len: u32][guest UTF-8]
       [module_len: u32][module WASM] ... in manifest order
```

Both signatures are made over the same preimage — `DOMAIN_manifest ‖ suite ‖ ed_pk ‖ ml_dsa_pk ‖ BLAKE2b-256(body)`, where `DOMAIN_manifest` is `"seedkernel-manifest-sig-v1\0"` (§16.1), prepended before signing but not stored. The disjoint prefix means a bundle signature can never double as a guest `node/sign` or channel-handshake signature over the same bytes (§14). There is no canonical-JSON step: the envelope carries the exact authenticated bytes and the verifier parses exactly what it checked.
...
**Only manifest suite `0x02` is supported.** Other ids are refused. Suite ids are not reused; a future manifest suite requires a new id (§14.1).
...
**Where the primitive comes from.** One freestanding wasm module (`browser/mldsa65.wasm`, built from the pinned `pq/mldsa-native` submodule by `scripts/build-mldsa.mjs`; `npm run build:pq` rebuilds it and `mlkem768.wasm` alongside it, needing the submodules checked out and a clang with the wasm32 target), fetched by the browser, read by Node and instantiated under wazero by the native binary — the same arrangement Ed25519 has through libsodium.wasm, and for the same reason: a bundle one node admits, every node must admit, so the verifier is compiled once and shared rather than reimplemented per target. The module has no imports at all (randomness and the FIPS 204 context are arguments), so there is no per-target host glue that could differ. `bundle.ts` names only the method `ml_dsa65_verify_detached` on the crypto object; a host that supplies none refuses `0x02` rather than falling back.

**The author id is derived, not carried.** It is `genesisHash(DOMAIN_manifest_author ‖ suite ‖ ed_pk ‖ ml_dsa_pk)` — 32 bytes, like an Ed25519 key, so every policy entry, revocation and freshness mark is written against one fixed-width identity. One suite means one derivation: nothing downstream has to ask which rule produced the 32 bytes it holds. Two reasons it is the hash and not simply the Ed25519 key:

- **Otherwise hybrid signing buys nothing at the moment it should pay.** An attacker who eventually breaks Ed25519 forges that half and generates a *fresh* ML-DSA key for the other. Both signatures verify; if the id were the Ed25519 key, the id would be unchanged, and the forged bundle would be admitted as the real author's, its policy pin and version lineage included. Hashing the whole key set makes the identity unreachable without both private keys — which is the property "hybrid" is supposed to name.
- **The id's width is load-bearing far outside the envelope.** A suite that widened it would change the policy file format, the revocation set and the freshness key at once.

A later suite deriving its id the same way would face the same consequence — an author moving to it would get a **new identity**: a fresh freshness lineage and a new policy entry. That is the honest reading rather than a wart, since a manifest signed under a different suite is a *different* statement about who signed. It is also why each multi-key suite writes its own derivation rather than parameterising this one: its key set is a different shape, so there is nothing to share but the mistake of deriving two identities the same way.

**One seed is the whole author.** `hybridAuthorKeysFromSeed` derives the Ed25519 half from a 32-byte seed directly and the ML-DSA-65 half from `genesisHash(seed ‖ AUTHOR_MLDSA_SEED_LABEL)` (§16.1). The label is a KDF tag, not a signing prefix (no trailing NUL), and it is **frozen**: changing it re-identifies every author built from a seed. The function lives in the offline-only `bundle-author.ts` rather than in each build script because a copy that drifts fails as a *changed author id* — every policy pin misses and every freshness lineage restarts, which no test catches when each copy agrees only with itself. Pass the 32-byte seed, not libsodium's 64-byte secret key.

**Manifest fields.**

| Field | Type | Enforced? | Meaning |
| --- | --- | --- | --- |
| `app` | string | **yes** | Names the coherent set, and is the slot's key: one slot per label on a node, whoever authored it (§12.10). It names the slot's filesystem and signing scopes and is what `replaces` and the operator's `uninstall` select; with the author it keys the freshness mark. Claims route directly to the complete slot, and modules are private to it (§3). Non-empty and at most 255 UTF-8 bytes (the signing scope's length byte); otherwise free text, since the filesystem prefix is a hash of it. |
...
| `modules[]` | `{name}` | yes | One entry per private WASM transform, in the order its bytes appear in the body. `name` is the bare name this slot's guest passes to `host.call`; there is no bind name or global namespace. Unique within a manifest, beginning with an alphanumeric character and restricted to `[A-Za-z0-9_-]`. Freely zero-to-many; a guest-only app declares none. A module cannot call another module, so composition remains the guest's job (§4.2). |
...
| *(both `protocols[]` and `services[]`)* | | | Two **claim maps**, not one vocabulary: one owner per ACTIVE name *per map*, so no second slot can shadow an owner in either, while a replacement atomically takes over its predecessor's slot and whole claim set. A name may appear in both lists, which says "reachable by a peer *and* by a co-resident guest" — two reaches to one owner, which two maps express without a rule. A claim grants nothing — which list it sits in decides reach, not any leading character; no spelling is reserved to the host. |
...
| `guest.requires` | string[] | **yes** | Everything this guest reaches: the host SERVICES it is granted (§12.2), by service name and never by method, and the local service ids it calls on a co-resident guest (§12.10), in one list. A name in `HOST_SERVICES` is a service; anything else is a local id, held to the claim charset. A method name (`fs/get`) is refused with the service to declare instead, and a local id may not live in a host namespace or collide with one of this bundle's own module names, because a `host.call` name means exactly one declared thing. |
| `guest.config` | JSON object | shape only | The app author's signed configuration, injected unchanged as `const APP`. Its schema belongs entirely to that app and is opaque to the runtime; the one shape the runtime insists on is an **object**, since a guest reads config by name and a signed scalar would leave every `APP.x` silently `undefined` rather than fail the load. Absent ≡ `{}`. Facts the runtime already derives do **not** belong here — the runtime's own facts reach the guest from the host (the node's public key in `HOST`, the slot scope `node/sign`/`node/verify` apply), never as restated config (see below). |

**Why `requires` and `config` live inside `guest`.** Both are the guest's alone: `requires` is its entire reach (§12.2, §12.10), and `config` only ever becomes its injected `APP`. WASM modules carry no authority, call nothing, and read no config, so neither has meaning at the top level — nesting groups the app's authority with the app's program, and "no authority ⇒ empty `requires`" is the schema's shape rather than a rule prose has to state and every target has to honour.

**Install algorithm** (`install`). Install is host code, so failures here **throw to the operator** — §3's "an unbound name resolves to an empty response" is about resolving a name, not about loading a local file.
...
1. Read the fixed-width suite, keys and signatures. Hash the whole remaining body and verify both signatures over the domain, suite, keys and body hash. Invalid ⇒ reject.
2. Parse and validate the authenticated manifest, then read the guest and modules in manifest order. Truncated fields or trailing bytes ⇒ reject.

*admit* — governance, the one predicate:

3. Ask the host's own gates of the freshness store, for every bundle so no posture can lose them — whether the author key is written off (§12.5), then the persisted `(author_pk, app)` high-water mark (absent ⇒ −∞). For an ordinary app, then call the admission predicate once: `admit(v: VerifiedBundle) → bool | Promise<bool>`. Return `true` to admit, `false` to reject, or throw to reject with a reason. The predicate is a pure function of the verified bundle: the store's facts are the gates' to read, and they have already answered. It IS the policy (§12.5) — a file-backed author allowlist, an interactive consent dialog, or "the bundle my operator handed me" are three constructors of it. An absent app predicate denies ordinary apps; link bundles require explicit transport selection instead.
...
5. Read every private module's declared memory and table limits before instantiating any module; enforce both per-module and aggregate bounds.
6. Build every private module off to the side and validate the §4 exports. If any fails, release the accumulated instances.
...
8. Re-check the host gates against the store as it now stands, persist the freshness mark for the version that successfully ran, then synchronously put this slot into the installed set — in the place of the one `replaces` named, or into a free one — and reproject every claim directly to it. Only after that commit is the selected predecessor disposed. A candidate that a newer version has since marked past, one whose author was revoked mid-load, and one whose mark cannot be written are all discarded here, leaving the running version unchanged.

**Splitting verify from construction is what makes consent possible.** An interactive client can show a bundle's author and metadata and wait before any realm or module instance exists. The single `admit` predicate sits between `verifyBundle` and slot construction: one predicate, one answer.

**Runtime facts come from the host, never from config.** `APP` and `LOCAL` carry only values explicitly supplied by the bundle author and this installation respectively. What the runtime itself knows reaches the guest from the host: the facts fixed for the realm's life in `HOST` — the node's public key and the budgets it is measured against — and its signing namespace through `node/sign` and `node/verify`, which apply the slot-derived scope for it. An author who baked the scope into `APP` would be restating a load-time fact at build time, and a copy that silently disagrees fails as signatures that verify nowhere with nothing naming the cause — the same one-file rule the `DOMAIN_*` family follows (§16.1). Signing and verification happen under the runtime's own derivation, which neither config value can touch.

**Module scoping is structural.** A slot captures one private module set and hands only that value to its guest seam. The guest calls a logical name such as `"codec"`; no label is supplied or looked up, and there is no wider namespace it could escape into. These calls need no grant because the modules are code from the same verified bundle (§12.1).

**Admission is a step inside install, not a separate component.** Install hands the target every verified module in one `build` call (§3.1). There is no per-module callback or outcome: trusting an author means trusting the complete set they signed, and the set either becomes a slot or is released.

**The policy needs no ownership state.** The slot's key is the verified manifest's `app` label and a claim points to the resulting slot. The policy is a pure function of the bundle; module ownership cannot drift because modules never exist outside their slot.

**One authentication, one authorization.** The bundle signatures authenticate the entire set (`verifyBundle`); the admission predicate (§12.5) authorizes the bundle — one predicate, one answer, between verify and install. The signed body binds every module's bytes and position to the manifest. There is no separate content-hash check.

**App configuration has three named provenances and one-app scope.** `HOST` is the runtime's own, fixed for the realm's life: the node's public key (`identity`, 64 lowercase hex — the key `node/sign` signs with, so a guest reads the identity it publishes at load rather than asking for it) and the budgets this realm will actually be measured against (`maxOutstandingHostCalls`, `maxOutstandingHostCallBytes`), told to the guest rather than discovered by being refused. The budgets exist so an app can *pace* itself — a guest fanning out concurrent `host.call`s windows them to the advertised byte budget, and a bound that would otherwise arrive as a hard rejection mid-PUT becomes ordinary backpressure. They are advice, not enforcement: the ceilings still refuse a guest that ignores them. Nothing an author or operator writes can change `HOST`, since it merely reports what the host already decided. `APP` is exactly the signed `guest.config`; the host never merges into it. `LOCAL` is exactly the optional JSON object supplied to this particular `install(blob, { localConfig })` call. The host passes it unchanged for every slot. The transport's network key and contact secret are ordinary transport config, while the node's identity arrives in `HOST`. The object is not retained by the host and cannot reach another bundle loaded beside this one. Both `APP` and `LOCAL` are objects and both are always defined (absent ≡ `{}`), so a guest guards keys, never the binding. A guest validates and combines the two objects itself, in code its own author signed. The shipped transport resolves each of its policy values as `LOCAL.x ?? APP.x`: the bundle signs its defaults, while `bootShell({ transport: { config } })` passes operator overrides through the automatic transport load's ordinary `localConfig` path. It **refuses at realm evaluation** — and so fails the load — if any of them resolves to something other than a non-negative finite number, because each one bounds a resource and an absent bound does not fail the comparison that applies it, it makes that comparison always false. A transport bundle carrying no `guest.config` is therefore refused rather than run uncapped. Realm memory, execution deadlines and admission remain outside both.

**Bundle freshness.** `version` is an enforced monotonic non-negative safe integer, not a label: the downgrade guard (steps 3 and 8) refuses any bundle whose `version` is below the persisted `(author, app)` high-water mark, so an older signed bundle — a stale relay copy, or a confused provisioning step handing over yesterday's build — is rejected as a downgrade. The whole bundle loads wholesale every boot, and neither guest nor modules carries a per-item version, so `version` is the single downgrade guard for the set. The mark is host-local persisted state; a deliberate rollback is an out-of-band operator action (the operator is the TCB, §14). The store file is `{ "marks": { "<author hex>:<app>": version }, "revoked": [ "<author hex>" ] }`; a bare marks map is refused because it lacks the required guard fields. Only a genuine missing file is first boot: malformed JSON, missing or wrong-shaped guard fields, invalid identities or versions, and read errors all fail closed. Unknown top-level keys remain the lenient case precisely because ignoring one discards no guard that was ever earned.

**The transport is keyed no differently.** Versions are an author's own lineage, so the bundle standing as the node's transport carries the same `(author, app)` mark as any other and there is no second floor keyed to the slot. One would cost what it bought: every author of a transport would share a single version line with no owner, and replacing A's v5 with B's transport would require B to number above a sequence B does not control. What it would buy is refusing B's stale v1 after A's v5, and that case needs someone other than the operator choosing which signed bundle replaces which. Nothing does: the transport changes only through boot selection or an install naming the owner it retires (§12.5), so the operator's explicit selection is already the decision a slot floor would second-guess. What `version` cannot express is that the signing key changed hands — see revocation (§12.5) and §14.

### 12.5 The admission policy

Admission (§12.4) runs after verification and before slot construction. Every bundle clears the host's gates — revocation, then freshness — before any app predicate, and again in the synchronous commit window, where the facts they read may have moved. Revocation comes first so a written-off key never reaches an interactive consent dialog. The gates are the host's rather than the operator's because they are invariants, not posture: an `admitAll` node still has a downgrade guard.

`install(blob, options)` admits ordinary apps through `admit(v)`. Naming no `replaces`, it takes a FREE label: a blob whose `app` label is already installed is refused by name, whoever authored it, and so is one requiring `link`. An absent app predicate is `denyAll`. `authorAllowlist`, interactive consent, and `admitAll` express app trust; further checks, such as sensitive claim owners, compose with them as ordinary functions.

**Contests are not gates.** Whether a claim, or the raw-link binding, is free is a question about the node rather than the bundle — the same manifest is admissible on a node where nothing holds them — so the slot table answers it before the candidate's code runs and again at commit (§12.10), never the predicate. That is what keeps `admit` a pure function of the bundle.

**Replacement is the same install, naming the slot it retires — and is the only way to displace one.** `install(blob, { replaces: app })` runs the identical sequence and atomically takes over that slot, even across authors or app names. So there is one operation and one field of variation, rather than two entry points to keep in step: an app's own next version names its slot exactly as a different author's candidate would. Ordinary app candidates still require app admission. A candidate requiring `link` is authorized only if the named predecessor owns `link`; replacing an ordinary chat app cannot acquire raw sockets. Same-author transport updates are this operation like any other.

A replacement may take the named predecessor's claims and label but cannot take unrelated claims, nor a label a third slot already holds. Only claims declared by the new manifest remain; claims it drops are released. The target is captured as an exact slot at call time and checked again at commit, so a concurrent update, replacement, uninstall, or shutdown makes the stale replacement fail. Construction, verification, admission, freshness, or persistence failure leaves the current owner running. Replacement works at the slot ceiling because it retires one slot as it installs one.

The filesystem and signing scopes belong to the label, the freshness history to the author. A replacement keeping the label keeps its data and signing scope, whoever authored it; one changing the label moves to the new label's, and the old label's keys stay on disk for whatever installs it next. A different author's v1 is independent of the predecessor's v50. Old invocation handles become invalid on successful replacement.

**The initial transport is selected at boot.** `transport.bundle` defaults to the embedded artifact; selecting those bytes authorizes their declared `link` service independently of app policy. Boot requires that bundle to declare `link`. `transport: false` (or omitted on `bootShell`) creates no transport. The CLI enables networking only with `--listen`, `--ws-listen`, or `--peers`; `--transport` selects the boot blob; it and `--contact-secret` are refused without one of them. After uninstalling the transport or replacing it with an ordinary app, starting another transport requires a new boot.

The selected transport is trusted with channel plaintext, session keys, signing in `DOMAIN_link_scope`, and peer attribution through `link/deliver`. Live replacement keeps the node identity and listeners and nothing else (§12.10).

**The signed service list is the authority declaration.** `reachesLink(manifest)` tests whether `guest.requires` contains `link`; the installer, exclusive binding, wiring, and signing scope use that same fact. Unknown services remain invalid manifests. Future services needing special authorization require an explicit host rule.

The policy file is `--policy <allowed-keys.json>` (`host/policy.ts`):
...
`authors` is required and contains the closed set of ordinary app authors. An empty array denies all apps. Malformed author ids, unknown keys, and malformed JSON fail boot. The transport is selected at boot, not here. Trusting an author trusts every module and guest their signed bundle carries; verification has already authenticated all of those bytes.

**Pin sensitive app claims in the app predicate.** The JSON policy reserves no protocol or service names. An embedder can add owner checks to its predicate, reading the verified author, `app` label and signed `protocols`/`services` lists; [CLIENT.md](CLIENT.md#the-assembly-is-an-export) carries an example. These checks apply to ordinary apps. Link bundles are explicitly selected at boot or by replacement of the link owner and bypass app predicates. A claim pin reserves ownership, not availability, and changing admission rules evicts no already-installed slot.

**Omitting `--policy` denies ordinary app installs.** It does not disable an explicitly enabled transport. Network activation and app author trust are independent decisions shared by the Node and native CLI implementations.

**Revocation is host-side, and it is one action.** A module is a transform with no host imports (§4.2), so nothing in the sandbox can reach install — there is no revoke-message, and revocation is not something a bundle can carry.

What it answers is the case freshness cannot. `version` orders an author's own releases; it says nothing about whether the key is still theirs. A **stolen author key set** clears the freshness guard trivially — the thief signs `version + 1` — and lands in the same slot with the same declared claims. Every release after the theft is, to the host, an ordinary upgrade.

So the store that holds the freshness marks also holds a set of **written-off author keys**, read into `ctx.revoked` and answered before the version: a revoked key's bundles are refused whatever they contain and whatever version they claim. `shell.revoke(authorHex)` does both halves in one call — record the key, then uninstall every app it already landed, found by the author each slot was admitted under. Both the Node CLI and the native binary (§12.9) expose it as `--revoke <hex,…>`, alongside `--uninstall <app,…>` for the narrower case; a client embedding the host calls the method.

**Both halves, or neither.** Uninstalling alone leaves nothing to stop the thief's next bundle re-landing under the same label and reclaiming its protocols or services; recording alone leaves the compromised code running. Neither implies the other, and an operator performing them by hand — edit `allowed-keys.json`, call uninstall — can do one and not the other, or do them in the order that leaves a window.
...
**The check is composed by the host, not supplied by the operator.** It is a predicate like any other, reading `ctx.revoked` — but it is the host that ANDs it in front of whatever the operator configured, so no posture can be a way to lose it: `admitAll` is still revocation-checked, and so is an interactive dialog that always says yes. One composition, in the shared host code, holds for every target and every delivery path.

**Recovery is a new key, not an un-revoke.** The runtime never removes a key from the set: it survives a later edit that puts the key back in `authors`, so re-admitting a compromised author takes more than forgetting why it was removed. It survives reboots wherever the store does — the Node CLI and the native binary persist it through the same atomic write as the freshness marks; the browser chat page holds both in memory, so there, as with freshness, a revocation lasts only as long as the page. An author who lost a key republishes under a new one, which starts a new freshness mark and may take the label and names released when the old app was revoked — inheriting the label's data and signing scope, including whatever the revoked code wrote there. An operator who distrusts that data deletes the label's keys before reinstalling. Genuinely undoing a revocation means editing the store file out of band — the same escape hatch as rolling a freshness mark back, and the same reason it exists.
...
Every `guest.requires` entry is signed, checked against the host service catalog, and enforced by the guest seam (§12.2). Admission requires explicit transport selection for bundles declaring `link`; ordinary apps use the app predicate. `node/sign` is available only through the slot-derived scope, never as a raw node-key oracle; filesystem, timers, and other declared services remain confined to the admitted slot.

### 12.6 Node↔node transport: channel identity binding

**Everything in this section is the transport bundle's guest program** (`transport/src/*.js`, §1) — the handshake, the record layer, the link router and the request/response frame codec — not host code. It reaches sockets through the `link` service and is reached through the local service id it declares under `services` (§12.10), and the host side of that is one driver, `host/transport-host.ts`, which owns the channels by the link id it mints and the listeners, and knows no protocol. The transport guest owns the correlation table, peer set, request facade and address book. What the driver hands a `link/open` to its socket factory is an opaque destination string; who is worth dialing, how often, and under whose contact secret are the occupant's. Its deadlines ride the one wake the ordinary `timer` service gives every realm (§12.3): each link, open correlation and `ready` waiter holds the monotonic time it ends on (`performance.now()`), the wake is armed for the soonest of them, and each wake walks them once, retires what is due and re-arms for the next — no timer per deadline and no polling. A deadline never fires early; queueing and CPU pacing can make it late, which a retention bound can afford. The wake is armed only while something waits, and a refused arm fails nothing: the next event asks again. A pending request also fails as soon as its peer loses its last routable link. What follows is therefore *content*: replaceable by a second signed bundle loaded the ordinary way (§12.4, §12.5), which is the property the rest of §12.6 exists to make safe. The *first* one is the exception, and cannot be anything else: a node has no network until it has a transport, so there is nothing to fetch one over except raw net to a peer it does not yet trust — which would open a metadata window before any channel exists to close it. It therefore ships inside the host artifact (§10.2), and what travels is the next one.

A real socket carries no trustworthy "from" field, so before a connection delivers frames the bundle runs a mutual challenge/response proving each end holds the private key for the pubkey it claims — the same binding applied to each WebRTC data channel (§11, §12.7). The channel is transport-agnostic over anything that delivers whole messages, and where the platform delivers none the bundle imposes them itself (§12.1): a length prefix over raw TCP node↔node, RFC 6455 over the same socket browser↔node, or a WebRTC `RTCDataChannel` peer↔peer as it comes (`services/net-rtc.ts`, §12.7) — same handshake, same frame plane, only the bottom byte-pipe swaps. Each framer checks its cap against the declared length **before** the body is buffered, so a peer cannot make a node allocate more than one frame; a platform-framed link has no declaration to check, so the delivered message is measured against the same cap on arrival. That cap is `MAX_HANDSHAKE_FRAME_BYTES` until the link authenticates and `MAX_FRAME_BYTES` after (§12.6.2). Every codec caps identically, and the boundaries a platform supplied are not a bound.
...
keys. Both AUTH signatures include that root in the transport's signed content, so their
preimages differ too (§12.6.3). The host applies only the constant link domain.
...
Sign(m) = Ed25519(DOMAIN_link_scope ‖ m)   — the prefix is the host's (§12.2)
h4      = H(h3 ‖ msg4)
...
`DOMAIN_link_scope`, and `h2` chains `DOMAIN_channel`, msg1 and msg2 — hence
...
outbound dial pins the peer it was dialed for: if msg4
...
`MAX_QUEUE_BYTES` and `maxPreAuthQueueSlices`, so neither large frames nor
many tiny frames can make a node hoard memory. A queue whose link goes before sending it —
a dial that dies, or one that loses the double-connect tie-break, which keeps the link the
smaller identity dialed — passes to another link to the same peer.
...
to close. Initiator-side identity-mismatch and peer-lint rejections at msg4 instead abort: the
...
- **Three deadlines.** `UNVERIFIED_TIMEOUT_MS` until msg1 opens, `HANDSHAKE_TIMEOUT_MS`
  for the rest, and `LINK_IDLE_TIMEOUT_MS` after that — an authenticated link
...
**Framer reassembly is linear, and there is one parser per link.** A TCP message arrives in arbitrarily small slices. Joining every slice onto one buffer makes a dribbled full-size frame cost a quadratic number of copies; keeping every slice as it arrived costs a view and a pinned chunk buffer per byte, times the half-open budget. The bundle's `ByteParts` (`transport/src/framing.js`) merges slices below 8 KiB into a doubling tail buffer so every byte moves a constant number of times and the live slice count is bounded by bytes/`MERGE_BELOW`. A slice at or above that size is kept as it arrived. The WebSocket framer serializes `push` so two parses never run over one buffer: `frames()` takes a frame before awaiting its decode, so a second parser would read the frame after it. Until the cap is raised, both framers also take one message at a time: `raiseCap()` lands only once msg4's step has *run*, so a frame riding the same segment is measured after that step — measured before it, against the pre-auth cap, a full-size first record would refuse a legitimate link.

**A request inherits the initiating host deadline.** The transport's `send` op carries no time field. The app's remaining handoff deadline crosses the `_net` realm call, covers the pre-auth pool and socket backlog, and remains attached to a deferred answer — it bounds the app's wait, while the transport's own turn runs on the transport's ceiling (§12.3), so an app out of time loses its request and never the link. Separately, the transport arms `requestTimeoutMs` (10 s by default, `0` disables) only to retire its own pending correlation; that timer cannot extend the inherited host deadline. Neither clock is waited out when the outcome is already known: a request no link could take — a peer with no address, a dial that opened nothing or died as the last way to its peer — fails at once, and a claimant's answer too big for one record, or none by the delivery's deadline, comes back empty rather than never. Socket `buffered()` is therefore host-only custody accounting for `LinkOutboundOwner`; it is not exposed as `link/stat`, and transport content cannot extend an owner's time because bytes happen to be moving.

**Requests waiting on a claimant share one window.** A request handed to `link/deliver` holds one of the transport realm's host calls and its bytes until the claimant answers, and every record open, seal and teardown on every link draws on that same advertised budget (`HOST`), where a refusal takes a link down. The transport therefore admits a decoded request only inside a window of that budget less room for one maximum-size record open and its plaintext, weighing each request as its bytes plus the budget's bytes per call so one sum bounds both ceilings. A peer with nothing waiting may use any room left; a peer already waiting leaves one maximum-size request's room to a peer that is not, and stops at an equal share among the peers waiting. A request past that is answered empty at once, the answer an unclaimed one gets. This bounds what one peer's requests can hold, not what they cost: an admitted request still runs to its deadline, and CPU across peers stays unscheduled (§12.3).
...
- **`MAX_INBOUND_HOLD_BYTES` and `MAX_INBOUND_HOLD_SLICES`** are one
...
- **`MAX_OUTBOUND_QUEUE_BYTES` and `MAX_OUTBOUND_QUEUE_SLICES`** bound
...
- **A teardown must not queue behind the wire it is ending.** The occupant runs one work
  ...
  synchronously first; the parked write fails, and the queued teardown then runs. The link
  leaves routing in the same breath rather than at that teardown: until then it could only
  drop a frame routed to it, while a fresh dial could carry it.

#### 12.6.2b One master seed, one identity

A node stores **one** secret: a 32-byte master seed. Its signing keypair is derived from
that seed under a distinct, versioned label (`services/subkeys.ts`) — `channel` — and that
keypair's public half **is** the node's identity: the peer id, what `senderPk` carries, and
what `HOST.identity` carries on every target. The master signs nothing itself, and
...
`DOMAIN_link_scope`, an ordinary app's `node/sign` binds
`DOMAIN_guest ‖ app` (§12.2) — one pair of names whose scope the slot picks by
...
| **Network key** | per deployment | **no, public by design** | Which network this transport belongs to. Honest transports on different network keys cannot link. Transport-enforced isolation — staging cannot reach production — not access control. |
...
**Peer admission depends on the transport.** The transport applies the configured peer predicate to signature-verified identities. A malicious transport can bypass that predicate or fabricate attribution, so the predicate does not constrain the transport itself. Host service grants and resource bounds still apply (§14).
...
`bootShell(opts)` (`host/shell-core.ts`, `./shell-core`) assembles the host and returns its `Shell` handle plus the channel adapter. Node's `bootNodeShell(opts)` (`host/shell-node.ts`, `./shell-node`) is a platform adapter over it: it selects Node crypto, `NodeFs`, file-backed freshness and `NodeChannelFactory`, and adds file-path bundle loading. It is convenience for a Node application, not a second assembly. The CLI over it is `host/cli.ts`:
...
**The CLI is shared code, not a per-target wrapper.** `runCli` (`host/cli.ts`) owns the flag set, the defaults (`--dir ./data`, `--key ./seedkernel.key`), the deny-all reading of an absent `--policy` (§14), the order — remedies, then the bundle, then the one-shots, then serve (§12.5) — and every line printed. A target supplies a `CliHost`: files, one console line, raw stdout, entropy, and "stand a node up on this platform". That is five members, none of which decides anything, and it is the whole difference between running a node on Node and running one from the native binary (§12.9). Argument *tokenizing* is not the point of sharing it — a dozen lines that fail loudly — the flag set and the sequence are, because a decision made twice is one that eventually gets made differently. Unknown flags are errors.

`--transport` supplies a signed transport bundle from disk instead of the artifact the build embeds (`TRANSPORT_BUNDLE_B64`) — a node with its own selected transport, or a newer protocol than the binary ships (§12.6). `--contact-secret` names a *file* holding 64 hex characters (§12.6.3), never the secret itself: an argument is visible in `ps` output and shell history. `--local-config` requires `--bundle` and is that one load's `LOCAL` JSON; it is never node setup and never reaches the transport bundle. `--peers` is the transport's, and goes in as `transport.config.peers` on the automatic load rather than being taught to a driver afterwards — the address book is the occupant's (§12.10) — so a malformed peer reference fails before anything is listening. A request's deadline is the caller's, set by `--guest-timeout`; §12.6 has how the transport's own `requestTimeoutMs` relates to it. Once the node is up the flow waits for that cohort with an ordinary claim call, `Shell.call` on the id the transport bundle claims: a `null` answer is "nothing here is the network", which is what an operator who typed `--peers` on a node with no installed transport must be told.

**`--op` names an op the CLI does not know, and that is the whole of its app-facing surface.** One flag rather than one per operation, because an op is a name travelling in `handle`'s payload (§12.2) and the runtime passes it through unread: a storage `put` and a chat `render` are reachable the same way, and neither is spelled anywhere in the host. **stdin is the argument and stdout is the response** — `handle`'s ABI exactly, bytes in and bytes out — so nothing here decodes, formats, or knows an app's argument shape. Neither a flag per operation nor a *choice* of argument flag can avoid knowing it, since which one an operator needs is decided by the app; composing bytes is left to the operator's command line. `log` therefore goes to **stderr** on both targets, since an operator line landing in the middle of a response would corrupt a redirect. The op targets the app `--bundle` just loaded through that load's returned handle — not through "the only app", which a node with a network cannot mean, since its transport is an ordinary app too (§12.10).

A serving node that has loaded a bundle serves the app's *request* side from its confined realm: an inbound frame reaches the guest's **one** entrypoint, `handle`, with the authenticated sender prepended (`dispatch` → `realm.call`), and the host drives the app's local logic through the **same** `handle` by a loopback — `AppHandle.invoke(payload)` writes the host's own caller id (32 zero bytes) in front of bytes it never reads, so a peer frame and a host loopback both arrive as `handle([caller 32][body …])`. An app therefore has one op vocabulary rather than two, and it is the app's: application and local-service bodies after the caller use the CALLEE's format. Raw-link events are also delivered through `handle`, but their envelope and fields are host ABI (§12.2). A guest declares `handle` — nothing else is ever invoked. Clients that choose the common named `[opLen u8][op][args]` envelope take it from `seedkernel-wasm/op-frame` (`services/op-frame.ts`), the codec also used by the socket driver to construct raw-link events. The host never imports or interprets that codec; the CLI's `--op` encoder is an operator convenience built on it. A timer follows the same opacity rule: `timer/arm` stores the caller-supplied four-byte tag and returns it verbatim on the future host loopback. Both directions may `await`; the realm serializes them (§12.3), and the driver resumes on the promise the app's `handle` returned rather than answering inline — which is what lets an app's inbound handling be asynchronous at all. The host is application-neutral — it can host any signed app — and for a self-contained non-browser deployment the Go/native target ships it as a single binary (§12.9). seed store's WASM README has a complete storage walkthrough.
### 12.9 The native binary — the primary non-browser deployment

The §12.8 host runs as JS on Node or Bun, but the **recommended** way to run a node outside the browser is the **Go/native target** (`native/`, a top-level Go module): a single self-contained, cgo-free binary — `seedkernel` — with no Node, no Bun, and no separate JS engine to install on the box.

It is a **platform target, not a reimplementation.** All protocol and app logic stays shared TypeScript — the guest seam (§12.2), the transport driver and its socket seams (§12.6 — the protocol itself is not host code at all, but the guest program of a signed bundle), install and its admission policy (§12.4–§12.5), bundle verification (§12.4), the confined safe-js guest (§12.3) — the same code the other targets run, just hosted differently. Go supplies only the platform **primitives** the §1 table calls for; protocol is never re-derived in a second language (*Go grows with primitives, never with logic*).

This is enforced mechanically: the shared modules are compiled by `tsc` and assembled into **one** `native/host-shell.gen.js` by `scripts/bundle-native-host.mjs` (`npm run build:native-host`), which the binary `//go:embed`s and evaluates in QuickJS. Nothing under `native/` is a hand-written second copy. A single TypeScript adapter (`host/native-shim.ts`) satisfies the same `PureModuleLoader`, `FreshnessStore`, channel, and realm interfaces as JS by forwarding to Go's byte bridge, then hands them to the shared `bootShell`. Typechecking makes a missed target change a compile error. Verification, admission, scope derivation, realm construction, freshness arithmetic, claim replacement, and the deny-all default all remain in the shared modules — one implementation of each to audit.

**The assembly order is shared too, not just the parts.** `bootShell` (`host/shell-core.ts`) is the one way to a `Shell`, and it owns verification, admission, module construction, scope derivation, realm construction, freshness, atomic claim replacement, inbound dispatch, and transport selection. Each target supplies only `{ sodium, identity, modules, fs, freshnessStore, transport, createRealm }`, and every one of those but the first two has a default. The channel adapter remains platform integration state rather than a `Shell` member; it is wired once to the generic claim lookup. There is no layer beneath the assembly for a target to reach: transport selection and the load order are part of standing a node up, not a step a caller may skip. A guest that fails to stand or a freshness mark that fails to persist disposes only the candidate, leaving the running slot intact. Go therefore holds no boot sequence, admission rules, routing model, or operator flow of its own.

Native keeps a Go `map[string]map[string]*boundModule` behind opaque slot handles because wazero modules cannot be JS values. That map is target plumbing behind `PureModuleLoader`, not host state. Concretely the binary embeds and drives, over [wazero](https://wazero.io):
...
- **a prebuilt QuickJS** (quickjs-ng, `native/qjs`) — so the shared host JS runs unmodified with no native JS-engine dependency. QuickJS is synchronous, so Go owns the event loop (timers, the JS job queue, socket delivery). A round-tripping `host.call` — net or fs — returns a real Promise to the guest: the host's guest seam starts the work under a call id and, when it settles, the realm's own settle callback resolves the guest's pending Promise, and the shared loop pumps the guest realm so the awaiting entrypoint resumes — the same real-promise seam the Node/Bun build uses, driven by Go's loop instead of quickjs-emscripten's job pump. Go's own `fs` primitive is synchronous (it reads the local disk in the call); the wrap that makes it the async seam the shared code consumes is in `native-shim.ts`, not in Go, because adaptation is logic and Go grows only with primitives. The confined guest runs in a second, zero-authority QuickJS realm whose only seam is `host.call`; because the settle path is per realm, a node hosting two guest apps keeps their seams apart by construction. Both engine blobs — this one and the JS platform's `quickjs/` emscripten build — are compiled from the **same** quickjs-ng v0.16.2 pin (`native/qjs/build-qjs.sh`, `WASM/quickjs/build-quickjs-ng.sh`), so a behavioral difference between targets is a build difference, not a version difference.

`ws.wasm` is a private module of the transport slot, instantiated by the same builder as any other private module; the codec that drives it runs in the transport guest.
...
plus `__fs.open`, through which the shared CLI opens the configured data directory. Even the `--key` file is read and minted in the shared
...
Because the wire and the bundles are shared, a Go node and a Node/Bun node interoperate directly in one cohort — `put` on either, `get` on the other, in both directions, against the same signed bundle and genesis (verified end-to-end for seed store by `WASM/scripts/native-interop.sh`).

**Scope: the native target is a bundle-runner.** Its app path is the §12.4 bundle — load, verify, install the modules, run the guest — and its request path is transport bundle → claim routing (§12.10) → the app's guest `handle` entrypoint, which reaches its private modules by their bare names through `host.call` (§12.2). Both targets install code only from a signed bundle (§12.4), so the app-delivery surface is identical. There is no dispatch loop and no signature pipeline to keep in parity: routing is one claim → slot lookup (§3) and modules are restartable transforms (§4), so Go's only module-facing duties are staging input into a module's `scratch`, reading its output, and honoring a declared `scratchSize` (§4.1) — byte-identical to the JS host. Admission and policy (§12.4–§12.5), bundle freshness (§12.4), and the domain prefixes (§16.1) are the same shared TS both targets run in QuickJS; the host-applied prefixes on every signature the native binary makes or checks come from that one evaluated `domains.ts`, so every signed preimage is byte-identical across the cohort by construction, not by a hand-copied constant. The channel's own `DOMAIN_channel` format tag is not in that family and does not need to be: it lives inside the transport bundle (`transport/src/ake.js`), which is one signed artifact every node in the cohort runs.

**Size.** One file, ~7.5 MB stripped, cross-compiled to win/linux/mac with `GOOS`/`GOARCH` — nothing to install alongside it. The bulk is wazero's compiler backend (~4 MB) and the Go runtime (~2.4 MB); the protocol's own footprint stays tiny (§10.2). Against the JS host — which needs a Node/Bun install plus the lazily-loaded ~570 KB QuickJS engine — the native binary trades a larger single artifact for zero external dependencies, the right shape for a server or an appliance.
...
**A frame names a protocol, not an app.** What travels is a protocol id in the Transport req frame (§12.6) — a chat message carries one, a storage message carries its op. It never names an app, an author, or a module: those are node-local (§5), and a wire that named them would make every peer's install choices everyone else's business.
...
**The claim maps are a projection of installed slots.** There are two routing maps, one per audience: `peerClaims` from every verified manifest's `protocols`, `localClaims` from its `services` — plus the raw-link binding, below, which is claimed the same way but routes no calls. One owner per name *within* a map; the host recomputes all of them on each atomic slot replacement or removal, and nothing persists routing separately.

**Two rules fall out of that projection.** A replacement serves only its own new claims, so it may atomically change the whole set — an app's next version and a different author's candidate are the one case, since both arrive by naming the slot they retire; and a candidate contesting any active peer or local claim is refused before commit. Uninstall releases the removed slot's claims; it does not reveal a shadow claimant because none can be installed.

**An install takes over nothing it did not name — not a claim, and not a label.** Without `replaces` it lands only where nothing is standing; every takeover names its target, or uninstalls first. This makes ownership deterministic, keeps installation order from changing routing behind an already admitted app, and gives two concurrent installs of one app a refusal rather than a silent winner. The `app` label is exclusive the same way a claim is: one slot per label, whoever authored it.

**A claim selects a recipient without granting host services.** It cannot make unadmitted code run, widen a guest's `guest.requires` (§12.2), reach another app's fs scope, or let one app sign in another's namespace. It does select the app that sees decrypted peer input or local call arguments and produces the answer. A wrong owner can therefore breach confidentiality and application integrity despite remaining confined; uninstall cannot undo data already disclosed. Different nodes may choose different owners, and the channel authenticates the peer node rather than its selected app. The operator can reserve sensitive protocol and service names for approved owners — an author and an app label — in the admission predicate (§12.5). Collision checks alone protect an occupied name, not a free name before installation or after release.

**Two claim lists, two maps, two audiences.** A manifest signs `protocols`, what a PEER may reach, and `services`, what a CO-RESIDENT guest — and the host itself — may reach with one call. Both share one charset: alphanumeric-or-`_` first character, then alphanumerics and `._/-`, at most 64 bytes. Uniqueness is **per list**: a duplicate within one is ambiguous and refused, while the same name in both is not — it says "reachable either way", two reaches to one owner, which is more than a single union map could express and needs no rule to allow. Which list a name sits in is a fact the manifest STATES, never a fact its spelling implies — `_net` is a spelling convention this repo's own transport bundle uses for its local service id, not a host-known reservation, and a name with no leading `_` claims under `services` exactly as well as one that has it.
...
**Raw-link events bypass the claim maps; inbound requests do not.** Platform link events target the slot that owns `link`, without consulting either map (§12.2). The occupant that sees the plaintext is the one that attributes — holding the sockets *is* the authority to say which peer a request came from, because nothing else ever held the bytes — so `link/deliver(claim, attribution, payload)` hands the host one request, which it routes through `peerClaims` alone, and the answer resolves the call for the occupant to frame.

**The raw-link binding is a claim.** A host service with `events` (`services/domains.ts` — today only `link`) has an occupant, and the driver has one event sink, so two holders are not a composition: the second would take the node's sockets off the first while leaving its claims and realm in place — installed, routed, and silently off the network. Requiring such a service therefore claims it, in a third book beside `peerClaims` and `localClaims`, under the same rule and at the same two points: an install contesting the holder is *refused*, and the holder changes only by being uninstalled or by being the `replaces` of the install that takes over. One rule is the book's own: a *free* `link` is taken only by the boot's transport selection (§12.5), because it carries authority no app predicate grants. The driver follows the book — bound to the holder, released when nothing holds it — and the host wires `rawNet` only into that slot, so no owner token is needed.

**Dispatch has one owner kind.** The claim map holds only bundle slots — there is no second table and no identity beside a slot that can ever hold a name, so nothing above needed a platform branch. What that would otherwise leave out is an embedder's own view of a peer-inbound answer: the link occupant consumes that answer on its way back out to the wire, so an embedder whose own mounted app must paint what it just answered — seedchat's guest relaying its render bytes to the page hosting it — has no other path to those bytes. `InstallOptions.onInbound(claim, sender, answer)` is that one seam: fired once a PEER-inbound frame's answer to THIS load's slot resolves, scoped to the load that named it rather than to the host, so there is still no table, no owner and no name to contest. It is observation only — the callback cannot change what the caller receives, is never consulted for a host loopback `invoke` or a co-resident guest's cross-realm call (both already hold the return directly), and a throw from it is reported and swallowed rather than reaching the transport driver.

**A guest reaches another realm by the same call, and only a local service id in ITS OWN `guest.requires` is callable.** `host.call("_service", …)` resolves through `localClaims`, the host prepends the CALLER's 32-byte id, and the answer is what the callee's `handle` returned on a later turn. The id sits in the same signed list as the host services, so the one list an admission predicate reads is the guest's whole reach — the network included, since an app talks to peers by calling the transport's id.

**The host reaches a claim the way a guest does.** `Shell.call(serviceId, payload)` is `localClaims` resolved with the host's 32-byte caller id — the host half of exactly that routing, `null` when nothing claims the name. It is the third and last door into a realm, and the three have unambiguous audiences: `AppHandle.invoke` is slot-bound (a bundle that claims nothing still has one), `Shell.call` is local reach, `link/deliver` is peer reach. Waiting for a cohort, listing linked peers and teaching an address are ordinary calls through it, composed with the same op codec both ends already share (`services/op-frame.ts`). These service ops belong to the transport bundle; the host-defined host↔transport event vocabulary remains three pure socket events — `linkOpen`, `linkBytes`, `linkClosed` — and nothing peer-shaped.

**Replacing the transport is ordinary slot replacement, not a protocol.** The candidate, whatever its author and label, loads complete and offside, including its private modules, realm and scopes. The host passes that load's ordinary `LOCAL` config unchanged: the network key, contact secret, limits and admitted-peer lint are all the load's own configuration, and the identity arrives in `HOST`. The embedder re-supplies the desired network key on replacement; omission selects the public network. After the realm stands and its freshness mark persists, the host synchronously replaces the named slot, its claims, and the raw-link binding. The socket driver retains its listeners, which are the node's; after installation only events and raw `link` bytes reach the occupant.

**Transport replacement discards session keys and the address book.** Peers must reconnect, and the **embedder** must re-supply addresses: name the peers in the replacement load's `transport.config` (`peers`, the `pk[.secret]@dest` list in hex — `services/peer-addr.ts` `peersConfig`), or teach them afterwards with an `addr` call through `Shell.call`, which reaches the current occupant and retains nothing host-side. The cost is real and it is deliberate: a node that upgrades its transport and says nothing else dials no one until it does, though it still *accepts* — the listener is the node's, so inbound links are up the moment the replacement is published. Both in-repo callers already hold the list they would re-supply (`cli.ts` from `--peers`, and seedstore's `StorageNode.connect`), and it is the same list either path takes, so there is one spelling of a cohort and not two.
