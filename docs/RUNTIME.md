...
7. Derive `fsScope` and `signingScope`, then stand the realm over a seam restricted to `guest.requires` and wired directly to those private modules. A guest that does not compile rejects the candidate.
