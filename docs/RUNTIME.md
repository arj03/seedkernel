...
| Name | Serves | |
| --- | --- | --- |
| `node` (`node/sign`, `node/verify`, `node/identity`, `node/random`) | the node key, scoped signing/verification under THE slot's one scope — an app's own, the link slot's constant link domain — and the entropy source — what the *host* owns | |
