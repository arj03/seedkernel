...
| `link/config`, `link/open`, `link/send`, `link/close`, `link/stat`, `link/authenticated`, `link/down` | **`link` privilege only** — bytes over an opaque link id, the platform's whole contribution to the network, and the reports back to the binding that supplied one | |
