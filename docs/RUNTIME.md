...
"Capabilities" from here on mean one thing: the **manifest requires** (§12.2, §12.4) — exact `host.call` grants such as `node/sign`, `fs/get`, `timer/arm`, `link/*`, and reserved local service ids. `link/*` requires the operator's `link` grant. They answer "may this *app's guest* reach this backend or local service at all?" (WASM modules, by contrast, carry no capabilities at all — a pure transform reaches nothing but the input it is handed and the output it returns, §4.2.)
...
- **Raw links, structured service, and delivery.** `link/*` moves bytes over an opaque link id and binds platform link events directly to the slot holding that capability — one slot, since the driver has one event sink, so a second link-capable load is refused rather than allowed to take the sockets over (§12.10). The transport exposes its structured API under an ordinary local claim selected by composition (`_net` in the bundled setup). Inbound attributed delivery is the link slot's **return convention**, not a second privilege: the occupant that sees the plaintext is the occupant that attributes, and its `linkBytes` invocation may return `(claim, attribution, payload)` for the host to route. No claim spelling confers the privilege (§12.5, §12.10).
...
The `link/*` names are the **raw-link** capability. Raw events arrive directly at the one slot owning that capability; what a transport provides back is an ordinary local service claim. Attributed inbound requests are returned off that same slot — a `linkBytes` invocation answers with `(claim, opaque attribution, payload)`, the host's claim table routes it, and the answer is written back to the link occupant for framing. It reaches only what a peer may reach, so a bundle's own `_`-led claim is not among them (§12.10): the *same shell* routing that refuses a peer's call to a local service name decides the delivery.

**The node's immutable facts are an argument, not a name.** The host invokes the freshly stood link occupant's `handle` once — before the binding is published, with the host's caller id — carrying `[identity blob][network key blob][contact secret blob][limits u32 …][admitted peers blob]` as the `init` op's payload. It is the constructor's input: only events and raw `link/*` bytes reach the occupant afterwards, and nothing about it is re-readable. The address book is mutable node state and is not among the facts: it arrives as `addr` events, replayed after publication (§12.10). The shape is versioned by `guest.abi`, so removing or reordering a field bumps it and appending one does not (§12.4).
...
| `guest.requires` | string[] | **yes** | **Exactly the authorities and local services this guest may call** (`AUTHORITY_CALLS`, §12.2, plus reserved ids). The seam grants these name by name; the shell wires only their backends. Ungated `crypto/*` and the bundle's own module names are not declarable. The list is closed and may be empty. `link/*` requires the operator's `link` grant — inbound delivery rides that same slot and is never a name to declare; reserved local service ids carry no privilege. |
...
| `grants` | at least one of `authors` / `grants` | Who may hold each **privilege**, keyed on the privilege itself. The keys are the host's `PRIVILEGES` (derived from `AUTHORITY_CALLS`, `core/domains.ts`); a key that names none is refused at the boot, so an operator cannot misspell a grant into a node that holds nothing. There is exactly one, a deliberate decision apart from `authors` and never app code: `link`, the raw links the node's transport is built out of — the holder carries the node's whole network, signs the channel AUTH (§12.2), and is the one that attributes and delivers what those links carry, so a second grant would only restate the first. A bundle reaching the privilege must be admitted by the grant AND by the node's transport author pin as well (§12.9), and is not judged by `authors` at all; one reaching none is judged by `authors` alone. A policy may name either side without the other; one naming neither is refused at the boot. |
...
Raw-link ownership is an explicit authority; inbound routing is its return convention. Platform link events target the slot binding that owns `link/*`, without consulting the claim table. The occupant that sees the plaintext is the one that attributes: its `linkBytes` invocation returns `(claim, opaque attribution, payload)`, the host routes it through the claim table, and the answer comes back to the occupant for framing — holding the sockets *is* the authority to say which peer a request came from, because nothing else ever held the bytes. A bundle's `_`-led claim is reachable by no peer and no delivery, because the same shell routing refuses it whether the frame arrived off a link or from a co-resident realm; claiming a service name authorizes neither.
...
**Replacing the transport is ordinary slot replacement, not a protocol.** The same `(author, app)` loads a complete candidate offside, including its private modules, realm and scopes. After it stands and its freshness mark persists, the shell synchronously replaces that identity's slot, claims, and raw-link binding. The socket driver retains the address book and listeners; the incoming guest received the node's immutable facts — identity, network key, contact secret, the limits, the admitted-peer lint — from the host as its one-time `init` invocation before the binding was published, and only events and raw `link/*` bytes reach it afterwards; the address book comes as `addr` events, replayed by the driver after publication. Live links do not survive because their session keys were private to the outgoing realm, so an upgrade reconnects.
