...
| `version` | integer | **yes** | Monotonic version of the coherent set. A load whose `version` is below the persisted `(author, app)` high-water mark is refused as a downgrade — and, for a bundle claiming a `role`, below that slot's floor as well (see freshness below). |
| `role` | string | if present | The **slot** this bundle claims, or absent for an ordinary app. One of a closed set (`transport` today) — an unrecognized name is a rejected manifest, never an ignored field, because a slot claim that fell through would be admitted as an ordinary app. It is signed, so what a bundle claims to be is the author's statement, and two things key off it: admission (a slot occupant needs a `roles` entry in the policy, §12.5 — the app allowlist does not admit one) and freshness (the floor is keyed by the slot as well as by `(author, app)`). A slot is a role every peer must interoperate with, where an app is a node-local choice that contends with nothing (§12.10). |
...
4. **Revocation, then freshness.** Refuse outright if `author_pk` is in the host's written-off author set (§12.5) — the shell has already checked this before step 3, so reaching it here means a caller drove the loader directly — a stolen key satisfies the version check trivially, so this must be asked first or it is never asked. Otherwise read the persisted high-water mark for `(author_pk, app)` (absent ⇒ −∞) and refuse if `version < high_water` — a downgrade, nothing lands. A bundle claiming a `role` is checked against that slot's floor too, and must clear both. Otherwise the mark advances at the *end* of a fully successful load (see freshness below). Equal versions reload (an ordinary reboot); the mark is monotonic and never rewound, so once version N loads, nothing older ever loads again on this node.
...
**Bundle freshness.** `version` is an enforced monotonic integer, not a label: step 4 refuses any bundle whose `version` is below the persisted `(author, app)` high-water mark, so an older signed bundle — a stale relay copy, or a confused provisioning step handing over yesterday's build — is rejected as a downgrade. The whole bundle loads wholesale every boot, and neither guest nor modules carries a per-item version, so `version` is the single downgrade guard for the set. The mark is host-local persisted state; a deliberate rollback is an out-of-band operator action (the operator is the TCB, §14). The store file is `{ "marks": { "<author hex>:<app>": version }, "roles": { "<slot>": version }, "revoked": [ "<author hex>" ] }`; a file written before revocation existed held the bare marks map and is **refused with a migration message** rather than read leniently, since parsing it as "no marks" would drop every downgrade guard on the first boot after an upgrade and say nothing.

**A slot needs a second floor, keyed by the slot.** `(author, app)` is the right key for an app: versions are an author's own lineage, and two authors' apps that happen to share a name are unrelated things. A **slot** is the opposite — its occupant is a role every peer must interoperate with (§12.4) — so a floor keyed to whoever signed says nothing about the case that matters. If a policy trusts authors A and B for the `transport` slot, A's v5 landing leaves B's mark at −∞, and an adversary controlling the fetch path serves B's validly signed v1, with a weaker channel suite, and it is admitted as fresh. So `roles` holds one monotonic floor per slot, author-independent: a slot load must clear both it and the ordinary mark, and advances both. A store written before slot floors existed has no `roles` and needs no migration — it reads as no floor, which is exactly true, since nothing could have claimed a slot on a host that had none. (That is the difference from the revocation case above, where a missing key would have dropped guards that *had* been earned.) What `version` cannot express is that the signing key changed hands — see revocation (§12.5) and §14.
