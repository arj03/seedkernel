...
**Only services are grants, and only grants are declared.** A crypto primitive is a function of bytes the guest already holds, so it reaches nothing and there is nothing to grant: the guest calls it by name through one ungated op and the host resolves it in a catalog (§12.2). The same rule exempts a bundle's own module names: an app's modules are its own bundle's code, installed and verified with it, so calling one reaches nothing the guest does not already hold — the scope (one app's map) is the shape, not a grant. Neither is *declarable* either: `guest.requires` carries services and local service ids and nothing else, so the list an operator reads is exactly the bundle's reach. The reason is that neither `crypto/*` nor a module name can be missing — the primitive catalog is total on any host that has a guest seam, and a bundle's modules arrive inside the bundle — so declaring them would be a requirement on something that cannot fail, and a dozen such names would bury the two or three that carry real authority. What a guest needs from that half of the seam is `guest.abi`, which versions every name in it.
...
**One catalog, three kinds of name, told apart by declaration first and charset second.** Host services and host primitives are the host's; a name in THIS realm's own declared local services is another realm's; anything else is the asking bundle's own module. The dispatch asks the DECLARATION first: a name the realm's own `guest.requires` lists as a local service routes to the realm claiming it (§12.10), however it is spelled — an id is an ordinary claim and may carry a `/` like a wire protocol id does, so asking the charset first would split one vocabulary into two, and it can never shadow a host service because the loader refuses a `requires` entry whose head is a known service (§12.4). A leading `_` is a composition convention this repo's own bundles use, never a kernel-known spelling. What is left splits by charset: **every host name contains a `/` and a module name cannot** (`[A-Za-z0-9_-]`, §12.4), which is already signed and already checked at verify, so a `/` means the host table keyed by SERVICE (below) and anything else means this app's module map. `crypto/*` gets its slash from the `crypto/${name}` template literal over `PRIMITIVE_NAMES`; `HOST_SERVICES` is hand-written, so the seam refuses at construction any service method spelled without one — a bare host method would shadow every app's module of that name. There is no second framing: the module name *is* the seam's name argument, not a length-prefixed field inside the payload.

**Two of those three kinds are not capabilities.** The `crypto/` entries — and a bundle's own bare module names — are the *primitive* seam: the first is a flat map over opaque names resolved in the host's catalog (§12.1), the second reaches the bundle's own module map, and both are ungated by a rule rather than by omission, because a function of bytes the guest already holds — or code the bundle itself shipped — is computation, not permission. Every other host name belongs to a *service*: it touches the node key, the entropy source, the clock, a socket or the disk, and is gated through the domains below, at the granularity of the SERVICE the method belongs to, not the method itself. That split is why a new algorithm never appears in this table — the `crypto/` entries are derived from `PRIMITIVE_NAMES`, so adding one to that list extends this one.
...
| `timer/arm` | `[id u32][ms u32]` | (empty) — a fired deadline re-enters `handle` as an ORDINARY host loopback naming the `timer` op (body `[id u32]`), not a second caller id |
