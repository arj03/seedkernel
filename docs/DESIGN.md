...
**One wake per realm bounds timers by construction.** One armed wake and one four-byte tag need no timer table, no ids and no payload accounting. Deadline tables, callbacks and scheduling policy live in the guest's own heap, where the heap cap already bounds them.
