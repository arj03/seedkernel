...
| `./peer-addr` | The `pk[.secret]@location` grammar — `parsePeerSpec`, `parsePeerRef`, `parseHostPort`, `isHex64` — with no socket adapter under it. The one place a client takes the parser from, whether or not it opens a socket | the in-repo caller is `parseWsPeer` in [`net-ws.ts`](../WASM/host/net-ws.ts) |
