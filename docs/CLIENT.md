...
| `./peer-addr` | The same grammar as the row above with no socket adapter under it — for a client that parses a `pk[.secret]@location` string but opens no TCP socket | no client imports it directly yet; the in-repo caller is `parseWsPeer` in [`net-ws.ts`](../WASM/host/net-ws.ts) |
