...
*The bootstrap sequence that composes the onion: the host stands up the handler table, loads its admission policy, and grows the deployment from signed bundles.*
...
Bootstrap is the host's job, not the kernel's. The host stands up the table, then optionally composes the layers above it. In the reference shell (§12.8) the whole sequence is short — the table starts **empty** and grows only through signed bundles:

1. **Ready libsodium and construct the host.** The kernel is the host's own `handlers` map (§3), so there is no module to load: on its own the host is already a usable table — `register` and `callHandler` work with nothing else wired. Boot is "load libsodium, done."
...
```
SetHandler(name, patchedHandler)
```
