...
The registry holds only the current record, not a history — no `parent` chaining a name's versions; bundle `version` (§12.4) is the sole ordering guard. For why a register rather than a log — and where an app that needs verifiable lineage puts it instead — see §14.
