...
| Seam names | `crypto/*`, `node/*`, `fs/*`, `clock/now`, `timer/*`, `link/*`, `route/deliver`, reserved `_`-led local service ids, plus the bare names of the calling bundle's own modules (`codec`, `ws`, …) | guest seam (§12.2) | Guest↔host name identifiers, a flat catalog of opaque strings — the keys of the seam's dispatch table plus the asking app's module map — the same shape as `PRIMITIVE_NAMES`. No op numbers, no generated preamble, never sent between nodes. **Every host name contains a `/` and no module name can** (`[A-Za-z0-9_-]`, §12.4), so one `host.call` covers all three sources and the dispatch tells them apart by the name alone. Neither the `crypto/` prefix nor a bare module name is a grant at all — the primitive seam, not a capability: the first resolves in a fixed host catalog, the second in the bundle's own module map. |
| Manifest requires | `AUTHORITY_CALLS` (`core/domains.ts`) plus reserved local service ids | manifest `guest.requires` (§12.4) | Exactly the authorities and local services a guest may call, granted BY NAME. The seam refuses an undeclared call and the loader refuses an unknown authority. `link/*` requires the operator's `link` grant; `route/deliver` independently requires `route`. Reserved `_`-led ids carry no privilege and resolve through ordinary claim routing. Ungated `crypto/*` and the bundle's own modules are not declarable; `guest.abi` versions their seam. |
...
| Guest ABI | `8` | manifest `guest.abi` (§12.4) | Which host seam a guest was written against (`GUEST_ABI_VERSION`, §12.2). Required — every bundle declares a guest (§12.4); a value this host does not implement is refused by name, like an unsupported suite. Tracks changes to the seam's *shape* — the naming scheme of `host.call`'s first argument, a name moving across the sync/async line, a payload framing change — not the appending of new names, which a guest that never calls them cannot notice. |
| Transport | `link/*`, `route/deliver`, and an ordinary local service claim chosen by composition | manifest `guest.requires` + `protocols` (§12.4, §12.5) | Three independent facts. `link/*` is admitted by the operator's `link` grant and binds raw-link events directly to that slot — one owner, so a second link-capable load is refused rather than allowed to take the sockets over. `route/deliver` has its own `route` grant and submits `(claim, opaque attribution, payload)` to routing, reaching only what a peer may reach: never a bundle's `_`-led local service name. The service claim grants neither authority and has no kernel-known spelling. Freshness and replacement use the ordinary `(author, app)` slot rules. |
