...
The host also retains installed slots for administration and initiator-only bundles, but that collection is not a second routing model: dispatch is always one `claim → slot` lookup. A claim has one active owner. A candidate that contests it is rejected; a replacement atomically takes over its predecessor's slot and claims.

The `app` label is the slot's key: one slot per label on a node, whoever authored it. It names the slot's filesystem and signing scopes and is what `replaces` and the operator's `uninstall` select; the author appears only in admission, revocation, and the author's own freshness mark. Host invocation uses the slot-bound handle returned by a load. Labels are not module addresses and claims do not route through them.
...
These belong to the reference runtime (§12), not the §3 slot model — a different host could change them without changing that model, but they are wire- or ABI-visible to bundles and peers of *this* runtime. Its bounds implement three distinct laws: retained space uses continuous custody, one causal chain carries a monotone deadline, and fresh invocation roots need explicit rate control. A limit in one column must not be read as a guarantee in another.
...
| Seam names | `crypto/*`, `node/*`, `fs/*`, `timer/*`, `link/*`, this realm's own declared local service ids, plus the bare names of the calling bundle's own modules (`codec`, `ws`, `mlkem`, …) | guest seam (§12.2) | Guest↔host identifiers, never wire values. A name resolves to whatever the manifest declared it as — a local service to its claimant, a module name to the bundle's module — and anything undeclared to the host table. Install keeps them disjoint: every host name contains `/`, module names are `[A-Za-z0-9_-]`, and a local service id may spell neither. Bare module names are private to the asking bundle and carry no grant. |
| Manifest requires | `HOST_SERVICES` (`services/domains.ts`) + local service ids | manifest `guest.requires` (§12.4) | Everything a guest reaches, in one list. A host service is granted BY SERVICE: the seam refuses a host method whose service is undeclared, and install refuses an entry naming a finer method (`fs/get`) rather than its service (`fs`). Any other entry is a local service id (claim charset) reached on a co-resident guest over the same `host.call`, resolved at call time through the local claim map — so an id nothing claims yet is not a manifest error — and refused when it lives in a host namespace or spells one of this bundle's own module names, since a seam name has exactly one meaning. `link` requires explicit transport selection; inbound delivery rides that slot and is never a name to declare. Ungated `crypto/*` and the bundle's own modules are not declarable. |
| Host transforms | `blake2b-256`, `chacha20poly1305-ietf/{seal,open}`, `x25519/dh`, `random` | guest seam `crypto/` prefix (§12.1, §12.2) | Frozen compatibility table (`HOST_TRANSFORM_NAMES`, `services/domains.ts`), ungated because each entry is pure, and at its floor because the host itself calls each one. New pure computation is a bundle module, not a new host name. ML-KEM is the transport's bare `mlkem` module; XChaCha20 belongs to the application that uses it. |
| Transport | `link`, and an ordinary local service claim chosen by composition | manifest `guest.requires` + `services` (§12.4, §12.5) | Two facts. `link` is authorized by boot selection or by an install replacing its current owner (§12.9), and binds raw-link events directly to that slot — one owner, so a second link-capable install is refused rather than allowed to take the sockets over. Inbound attributed delivery is `link/deliver`, one of that service's own methods: the occupant that sees the plaintext is the one that attributes, and the host routes what it hands over through the PEER claim map alone, so a bundle's `services` local claim is unreachable there by construction. The service claim grants no authority and has no host-known spelling. Freshness uses the ordinary `(author, app)` mark; the slot changes hands only through an install naming the current holder, same author included. |
...
| `DOMAIN_guest` | `"seedkernel-guest-sig-v1\0"` | Guest seam `node/sign`/`node/verify` for an ordinary app (§12.2) | Domain-separation prefix for guest-obtainable signatures, followed by the host-derived scope `app_len u8 ‖ app` from the admitted manifest. Prepended before signing *and* verification (host-applied on both), never transmitted. |
| `DOMAIN_link_scope` | `"seedkernel-link-scope-v1\0"` | Guest seam `node/sign`/`node/verify` for the slot reaching `link` (§12.2, §12.6) | Constant domain-separation prefix for that slot's signatures, with no additional host-owned scope bytes. The transport binds its network key in the signed handshake content. The slot's scope, not a second name: the host derives one `SignScope` at load, and this is the link slot's. Host-applied on both sides like `DOMAIN_guest`, never transmitted. The host owns the separation and nothing inside it: what format is signed under this scope is the occupant's. |
| `DOMAIN_channel` | `"seedkernel-channel-id-v1\0"` | Transport bundle's handshake signature and transcript (§12.6) | **Not a member of the family above** — a format tag the transport bundle places in its own content, used to seed the transcript hash and prefix each identity payload `DOMAIN_channel ‖ root ‖ transcript ‖ own_id`. It sub-separates the transport's formats *within* `DOMAIN_link_scope`, so changing the handshake is a bundle update rather than a host change. Declared in `transport/src/ake.js`, not `services/domains.ts`. Not transmitted. |
...
| Master seed | 32 bytes, on disk | `subkeys.ts` (§12.6.2b) | The node's only stored secret. Signs nothing; derives the `channel` keypair under a versioned label. That keypair's public half is the peer id and the node's one identity — it signs through the one guest-seam sign pair (`node/sign`), whose scope the host sets from the slot: the link slot's `DOMAIN_link_scope` for the handshake, an app slot's `DOMAIN_guest ‖ app` for its own records. |
| `AUTHOR_MLDSA_SEED_LABEL` | `"seedkernel-author-mldsa-v1"` (no trailing NUL) | `hybridAuthorKeysFromSeed` (§12.4) | KDF label, not a signing prefix: `genesisHash(ed25519_seed ‖ label)` is the ML-DSA-65 seed of the same author, so one stored 32-byte seed is the whole key set. **Frozen.** The author id hashes both public keys, so changing the label re-identifies every author built from a seed — a dead freshness lineage, every policy pin pointing at nobody. |
| FS key charset | `[A-Za-z0-9._-]+`, minus `.`/`..` and Windows device names | `isSafeFsKey` (`services/fs.ts`) | Both backends map a key to a filename verbatim, so the charset is a consensus predicate, not a backend detail: two nodes disagreeing about it disagree about their contents. Device names (`CON`, `PRN`, `AUX`, `NUL`, `COM0`–`COM9`, `LPT0`–`LPT9`) are refused on every OS; Windows ignores the extension, so `NUL.txt` is still `NUL`. A scope is a prefix, not a complete name, and is held only to the charset. |
