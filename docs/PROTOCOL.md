...
| Manifest envelope | `[suite 1][pk 32][sig 64][json]` | `loadBundle` (§12.4) | Ed25519 detached signature over `DOMAIN_manifest ‖ suite ‖ json`. The suite byte is stored *and* signed, so a verifier may read it for field widths before verifying and still cannot have it edited under them (§14.1). |
...
| Algorithm suites | channel `0x02`, manifest `0x01` | msg1 byte 0 (§12.6) · manifest byte 0 (§12.4) | **Two independent namespaces on independent clocks** — channel (Ed25519 identity · ephemeral X25519 · contact secret · ChaCha20-Poly1305) and manifest (Ed25519 detached); never read one as the other. The channel is at `0x02` because the cleartext-identity genesis suite `0x01` was **removed, not disabled**: a node accepting both would have the concealment of the weaker one. Neither namespace is negotiated — one suite per link, one per manifest, unknown ids refused (silently, on the channel). Each is covered by the signature it accompanies, so a suite is chosen by an endpoint and never forced in flight (§14.1). |
