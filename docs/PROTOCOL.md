...
| `DEFAULT_MAX_MODULE_MEMORY_BYTES` | *host default* | Slot construction (`loadBundleModules`) | Ceiling on a module's declared initial *and* maximum footprint — linear memory plus its tables at `WASM_TABLE_ELEMENT_BYTES` each — read before instantiation (§4.1). Applied at that one call site, so every target holds its isolates to one number rather than repeating the rule per loader. |
