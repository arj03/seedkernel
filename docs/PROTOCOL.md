...
| Master seed | 32 bytes, on disk | `subkeys.ts` (§12.6.2b) | The node's only stored secret. Signs nothing; derives the `channel` keypair (whose public half is the peer id) and the `guest` keypair (guest seam `node/sign`) under versioned labels. |
