...
> **Memory is bounded at admission; compute is charged but uninterruptible.** They are two residuals rather than one, because only one of them has a mechanism to interrupt with.
>
...
>
> **Compute** is not, and cannot be here: WASM engines on the JS platform expose no fuel or timeout mechanism, so this protocol specifies none. What that leaves is narrower than "unbounded". Nothing on the message path does asymmetric crypto or recurses, and a module runs only when its app's guest calls it (§12.10) — a *synchronous* call, so the guest does not park and the time the module burns is **charged to the calling guest's budget** (§12.3). A module that returns is therefore bounded: the guest is killed at the first interrupt after the call. What is uninterruptible is the call itself — QuickJS ticks its interrupt handler only while running bytecode — so a module that never returns holds the single-threaded host forever, and a permissive policy (§12.5) multiplies that across many installs. Deployers exposed to runaway modules should run the host in a Worker with a watchdog, or pre-validate bytecode in the admission policy (forbid unbounded loops) before installing. The asymmetry with the confined JS guest is the tick, not the accounting: QuickJS offers an interrupt hook where WebAssembly on this platform offers nothing.
...
| Transport | the four `link/*` names and the reserved `_net` claim | manifest `guest.requires` + `protocols` (§12.4, §12.5) | What makes a bundle the node's transport, on the one install path: the shell verifies it and admits it under the policy's `link` grant (never the plain `authors` list); the claim of `_net` — which `verifyManifest` grants only to a bundle reaching `link` — is what the routing serves it by. It is an app in every other respect, reached by the same call an inbound frame uses and replaced by a later load of the id. The manifest has **no role field** — the requires and the claim already carry the fact, and the signature already covers them. Naming them is not a way in: it puts a privilege in the set the author must be granted. Freshness is the ordinary `(author, app)` mark, so each author of the transport keeps their own version lineage. |
| Manifest envelope | `0x02`: `[suite 1][ed_pk 32][ml_dsa_pk 1952]`<br>`[ed_sig 64][ml_dsa_sig 3309][json]` | `loadBundle` (§12.4) | Fixed width. Signs `DOMAIN_manifest ‖ suite ‖ ed_pk ‖ ml_dsa_pk ‖ json` with **both** keys and requires **both** to verify. The suite byte is stored *and* signed, so a verifier may read it for field widths before verifying and still cannot have it edited under them (§14.1). |
...
| Algorithm suites | channel `0x02` · manifest `0x02` | msg1 byte 0 (§12.6) · manifest byte 0 (§12.4) | **Two independent namespaces on independent clocks** — channel (`0x02`: Ed25519 identity · ephemeral X25519 · contact secret · ChaCha20-Poly1305) and manifest (`0x02`: hybrid Ed25519 + ML-DSA-65, both required); never read one as the other. Each namespace holds exactly one live value, and in both cases the genesis suite `0x01` was **removed, not disabled** — on the channel because a node accepting both would have the concealment of the weaker one, on the manifest because a second envelope kept alive for a population that never existed is machinery, not compatibility. They moved for unrelated reasons, which is what independent clocks look like in practice. Neither namespace is negotiated — one suite per link, one per manifest, unknown ids refused (silently, on the channel). Each is covered by the signature it accompanies, so a suite is chosen by an endpoint and never forced in flight (§14.1). |
...
| `DOMAIN_guest` | `"seedkernel-guest-sig-v1\0"` | Guest seam `node/sign`/`node/verify` (§12.2) | Domain-separation prefix for guest-obtainable signatures, followed by the host-derived scope `author_pk ‖ app_len u8 ‖ app` from the admitted manifest. Prepended before signing *and* verification (host-applied on both), never transmitted. |
...
| Master seed | 32 bytes, on disk | `subkeys.ts` (§12.6.2b) | The node's only stored secret. Signs nothing; derives the `channel` keypair under a versioned label. That keypair's public half is the peer id and the node's one identity — it signs the handshake and, under a different domain, guest seam `node/sign`. |
...
| `DEFAULT_REQUEST_DEADLINE_MS` | `10000` ms | transport driver (§12.6) | How long one request may take before it settles as unreachable, for a caller that names no deadline of its own. A **deadline is per request, not per node**: only the caller knows whether it sent a 200-byte control message or a 4 MB block, so `request(to, proto, payload, deadlineMs)` takes one and this is merely the fallback (operator default `requestDeadlineMs`, CLI `--request-deadline`). An app names it in the `send` op it calls on the transport, or leaves it 0 and takes this — which is why it is generous rather than tight. |
| Guest execution budget | `5000` ms | Confined realm (§12.3) | Per entrypoint invocation, measured over guest **run** time rather than wall clock: a guest parked awaiting a host seam spends none of it. Exceeding it interrupts the guest, which surfaces to the caller as a thrown error; on the native target the interruption terminates the realm rather than throwing inside it (§14). Operator-set (`guestDeadlineMs`, CLI `--guest-timeout`), not author-declared; `Infinity` — CLI `0` — disables it. The realm heap cap alongside it is 64 MiB (`realmMemoryBytes`, CLI `--guest-memory`, in MiB). |

**The `DOMAIN_*` family lives in one file.** The four prefixes above are a *family*, and the only thing they are for is disjointness: no signature made under one may verify under another, over any bytes, ever — and no derived id may collide with a signed preimage. That is a property of the whole set rather than of any member, so the set is declared together in `core/domains.ts` — where adding a member means reading the others on the same screen — and imported by the modules that sign (`bundle.ts`, `guest-seam.ts`). The transport bundle's guest program never sees one: it asks for a signature through `node/sign` and the *host* chooses the prefix from the asking bundle's slot (§12.2), so a prefix cannot be restated in content either. The Go/native target evaluates that same file through its generated bundles (§12.9) and reads its prefixes from the evaluated module, so every prefix on every target derives from this one file by construction, not by a copy that could drift. There is **no** hand-copied member anywhere.
