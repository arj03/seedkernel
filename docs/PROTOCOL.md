...
**Unbound resolves to nothing.** A name absent from the table is not an error and does not raise one: `callModule` returns no bytes, and the guest that asked through `module/call` reads an **empty response** (§12.2). The same shape holds one level up at the protocol bindings — an unbound protocol reaches no app, and the transport answers the request with an empty body rather than discarding the frame (§12.10). Nothing on either path is silently discarded, so there is no "drop" for the table to define; what the host does *not* do is produce unsolicited output, and every reply an app sends travels as a fresh frame under that app's own logic.
...
**Composition is the guest's job.** Chaining transforms — running one module's output into another, fanning out, doing I/O between steps — is the app's guest (or a host-side embedder), never a module's. A guest reaches its own modules through the cap-bridge's `module/call` name (§12.2); the host-side equivalent is `callModule(appKey, name, bytes)`. Because a module cannot call back, these compose without re-entrancy: each transform returns before the next runs.
...
| Capability domains | `node`, `net`, `fs`, `clock`, `timer` · mount-only: `link`, `transport` | manifest `caps` (§12.4) | A granted domain IS its prefix: the bridge refuses any `host.call` whose first path component is not one of the declared domains. An unknown domain throws at load. Only **authorities** appear here — each reaches something no confined module can hold. `link` (bytes over an opaque link id) and `transport` (the attributed peer, protocol id and correlation the transport provides back) are the transport's: a manifest declaring them is governed by `transportAuthors` and mounted as the node's transport rather than bound as an app, and one declaring some but not all of them is refused as malformed (§12.2, §12.5). Pure transforms are not domains: a guest calls them by name under the `crypto/` prefix (below), or reaches its own bundle's modules through `module/call` — both ungated (§12.1). |
...
| Transport mount | `guest.caps ⊇ {link, transport}` | shell (§12.4, §12.5) | What makes a bundle the node's transport, on the one install path: the shell verifies it, admits it under `transportAuthors` (never the app allowlist), and stands the driver up instead of binding protocols. The manifest has **no role field** — the caps already carry the fact, and the signature already covers them. Declaring them is not a way in: it moves a bundle onto the stricter list. Freshness is the ordinary `(author, app)` mark, so each author of the transport keeps their own version lineage. |
