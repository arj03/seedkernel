...
The candidate realm evaluates its guest's top level between steps 3 and 5. Until the slot is marked active, the seam refuses names that leave something behind — `fs/put`, `fs/delete`, `route/deliver`, and any reserved (`_`-led) id — because disposing the candidate is the only undo (`isIrreversible`, `core/domains.ts`). Reads, `crypto/*`, the bundle's own modules, `timer/*`, and `link/*` (which answer only the unpublished slot) stay open, so a transport candidate can still read `link/config` offside.
...
| `DOMAIN_link_scope` | `"seedkernel-link-scope-v1\0"` | Guest seam `link/sign`/`link/verify` for the slot reaching `link` (§12.2, §12.6) | Domain-separation prefix for that slot's signatures, followed by the node's 32-byte network key. Host-applied on both sides like `DOMAIN_guest`, never transmitted. The kernel owns the separation and nothing inside it: what format is signed under this scope is the occupant's. |
