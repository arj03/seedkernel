...
| `MAX_FRAME_BYTES` | `2097152` (2 MiB) | socket seams (§12.6) | Cap on one post-authentication frame, checked before buffering; the transport does not fragment, so it is also the largest application message. `ws.wasm` stages a whole frame in a scratch region, so raising it means rebuilding `ws.wasm` (asserted by `tests/transport.test.mjs`); a deployment may lower it with `transport.config.maxFrameBytes`. |
