...
| Node address | `pk[.secret]@[scheme://]host:port[/path]` | `parsePeerSpec`, `parsePeerRef` (§12.6) | The peer's Ed25519 identity, optionally that peer's contact secret, and where to reach it. The scheme and path are the WS edge's business — `wss://` asks for TLS, a path reaches a peer behind a reverse proxy — and a bare `host:port` is the common case. The pk is **routing** — it keys the address book so dial-by-identity has something to look up — and the secret is the **credential**. No long-term Diffie–Hellman key is published, so a PQ suite will not change the format. |
