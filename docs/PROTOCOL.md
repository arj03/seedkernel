...
| Guest ABI | `2` | manifest `guest.abi` (§12.4) | Which host seam a guest was written against (`GUEST_ABI_VERSION`, §12.2). Required wherever a guest exists; a value this host does not implement is refused by name, like an unsupported suite. Bumped when an existing op changes what it returns — not when an op is appended. |
