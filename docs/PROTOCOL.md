...
| `DOMAIN_link_scope` | `"seedkernel-link-scope-v1\0"` | Guest seam `node/sign`/`node/verify` for the slot reaching `link` (§12.2, §12.6) | Domain-separation prefix for that slot's signatures, followed by the node's 32-byte network key. Host-applied on both sides like `DOMAIN_guest`, never transmitted. The kernel owns the separation and nothing inside it: what format is signed under this scope is the occupant's. |
