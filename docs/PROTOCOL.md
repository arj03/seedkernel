...
| Host transforms | `blake2b`, `chacha20poly1305-ietf/{seal,open}`, `x25519/dh`, `random` | guest seam `crypto/` prefix (§12.1) | Frozen compatibility table (`HOST_TRANSFORM_NAMES`), each name over its algorithm's whole standard interface (§12.2); why it is at its floor is §14. |
...
| Link messages | msg1 1,233 B, msg2 1,200 B, msg3 112 B, then AEAD records | transport bundle (§12.6) | Bare bodies with no type byte, each accepted only at its exact width. |
...
| Node address | `pk[.secret]@[scheme://]host:port[/path]` | transport bundle (`peerRef`, §12.6.3) | The shipped transport's grammar, not the host's: the shell passes references through unread. Peer id, optionally that peer's contact secret, and an opaque destination `scheme://host:port[/path]` — what `link/open` carries. `tcp://` is node↔node, `ws://`/`wss://` the RFC 6455 codec (the latter asking for TLS), and a path reaches a peer behind a reverse proxy. The transport reads a bare `host:port` as `tcp`. No long-term DH or KEM key is published. |
...
| WebRTC relay frame | UTF-8, NUL-separated: `h from to`, `o`/`a from to sid sdp`, `i from to sid candidate sdpMid sdpMLineIndex usernameFragment` | transport bundle (`rtc.js`, §12.7) | The shipped transport's signaling, forwarded verbatim by the relay to the rest of a room. Keys are 64 lowercase hex, `to` empty broadcasts a hello, `sid` is 16 hex. Every member of a room must speak it. |
| `rtcConnectTimeoutMs` | *transport default* | transport bundle (`rtc.js`, §12.7) | How long a negotiation has to open its data channel; the handshake deadlines start when it does. |
