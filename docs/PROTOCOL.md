...
**Naming convention for bootstrap handlers:** `name = "seedkernel.bootstrap.v1:" + canonical_name` — the **literal ASCII string**, not a hash. Names are opaque bytes, so nothing forces a hash; a readable name reads plainly in logs and keeps the bootstrap namespace independent of the genesis hash (only `bytes_hash` earns that dependency, §14). These names are host-seeded via `SetHandler` (§3.1) when a deployment wires a handler by hand, not admitted through the registry, so there is no install record to mix in. The chat shell uses the same helper (`deriveBootstrapName`) to derive an *app's* kernel name from its id, so two peers running the same app route to the same name (§11).
