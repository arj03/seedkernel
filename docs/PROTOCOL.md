...
| Seam names | `crypto/*`, `node/*`, `fs/*`, `clock/now`, `timer/*`, `link/*`, reserved `_`-led local service ids, plus the bare names of the calling bundle's own modules (`codec`, `ws`, …) | guest seam (§12.2) | Guest↔host name identifiers, a flat catalog of opaque strings — the keys of the seam's dispatch table plus the asking app's module map — the same shape as `PRIMITIVE_NAMES`. No op numbers, no generated preamble, never sent between nodes. **Every host name contains a `/` and no module name can** (`[A-Za-z0-9_-]`, §12.4), so one `host.call` covers all three sources and the dispatch tells them apart by the name alone. Neither the `crypto/` prefix nor a bare module name is a grant at all — the primitive seam, not a capability: the first resolves in a fixed host catalog, the second in the bundle's own module map. |
| Manifest requires | `AUTHORITY_CALLS` (`core/domains.ts`) plus reserved local service ids | manifest `guest.requires` (§12.4) | Exactly the authorities and local services a guest may call, granted BY NAME. The seam refuses an undeclared call and the loader refuses an unknown authority. `link/*` requires the operator's `link` grant; inbound delivery rides that slot and is never a name to declare. Reserved `_`-led ids carry no privilege and resolve through ordinary claim routing. Ungated `crypto/*` and the bundle's own modules are not declarable; `guest.abi` versions their seam. |
...
| Transport | `link/*`, and an ordinary local service claim chosen by composition | manifest `guest.requires` + `protocols` (§12.4, §12.5) | Two facts. `link/*` is admitted by the operator's `link` grant AND by the node's transport author pin (§12.9), and binds raw-link events directly to that slot — one owner, so a second link-capable load is refused rather than allowed to take the sockets over. Inbound attributed delivery is that slot's return convention, not a second grant: the occupant that sees the plaintext is the one that attributes, and the host's claim table routes what the occupant returns, reaching only what a peer may reach: never a bundle's `_`-led local service name. The service claim grants no authority and has no kernel-known spelling. Freshness and replacement use the ordinary `(author, app)` slot rules. |
