...
| Seam names | `crypto/*`, `node/*`, `fs/*`, `clock/now`, `timer/*`, `link/*`, this realm's own declared local service ids, plus the bare names of the calling bundle's own modules (`codec`, `ws`, …) | guest seam (§12.2) | Guest↔host name identifiers, a flat catalog of opaque strings — the keys of the seam's dispatch table plus the asking app's module map — the same shape as `PRIMITIVE_NAMES`. No op numbers, no generated preamble, never sent between nodes. **Every host name contains a `/` and no module name can** (`[A-Za-z0-9_-]`, §12.4): a name this realm DECLARED as a local service dispatches to the realm claiming it whatever its spelling (§12.10), and what is left splits by charset — a `/` through the host table keyed by SERVICE, anything else through the bundle's own module map. Neither the `crypto/` prefix nor a bare module name is a grant at all — the primitive seam, not a capability: the first resolves in a fixed host catalog, the second in the bundle's own module map. |
