...
| Node address | `pk[.secret]@[scheme://]host:port[/path]` | transport bundle (`peerRef`, §12.6.3); `parsePeerRef` for an `addr` call | The shipped transport's grammar, not the host's: the shell passes references through unread. Peer id, optionally that peer's contact secret, and an opaque destination `scheme://host:port[/path]` — what `link/open` carries. `tcp://` is node↔node, `ws://`/`wss://` the RFC 6455 codec (the latter asking for TLS), and a path reaches a peer behind a reverse proxy. The transport reads a bare `host:port` as `tcp`; `parsePeerRef` takes the caller's default. No long-term DH or KEM key is published. |
