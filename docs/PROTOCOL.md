...
>
> **Compute** is not, and cannot be here: WASM engines on the JS platform expose no fuel or timeout mechanism, so this protocol specifies none. What that leaves is narrower than "unbounded". A module runs only when its app's guest calls it (§12.10), and the call is *synchronous* — the guest does not park, so the time the module burns is **charged to the calling guest's budget** (§12.3), and a module that returns is bounded: the guest is killed at the first interrupt after the call. Only the call *itself* is uninterruptible, because QuickJS ticks its interrupt handler while running bytecode and during a WASM call it is not. The asymmetry with the confined JS guest is therefore the tick, not the accounting. §14 has the exposure that leaves and what a deployer does about it.
...
| Seam names | `crypto/*`, `node/*`, `fs/*`, `clock/now`, `timer/*`, `link/*`, the reserved `_`-led ids (`_net`, `_host`), plus the bare names of the calling bundle's own modules (`codec`, `ws`, …) | guest seam (§12.2) | Guest↔host name identifiers, a flat catalog of opaque strings — the keys of the seam's dispatch table plus the asking app's module map — the same shape as `PRIMITIVE_NAMES`. No op numbers, no generated preamble, never sent between nodes. **Every host name contains a `/` and no module name can** (`[A-Za-z0-9_-]`, §12.4), so one `host.call` covers all three sources and the dispatch tells them apart by the name alone. Neither the `crypto/` prefix nor a bare module name is a grant at all — the primitive seam, not a capability: the first resolves in a fixed host catalog, the second in the bundle's own module map. |
...
| Guest execution budget | `5000` ms | Confined realm (§12.3) | Per entrypoint invocation, measured over guest **run** time rather than wall clock: a guest parked awaiting a host seam spends none of it. Exceeding it interrupts the guest, which surfaces to the caller as a thrown error; both targets enforce it through QuickJS's own interrupt handler, so the overrun costs one invocation and the realm survives it (§14). Operator-set (`guestDeadlineMs`, CLI `--guest-timeout`), not author-declared; `Infinity` — CLI `0` — disables it. The realm heap cap alongside it is 64 MiB (`realmMemoryBytes`, CLI `--guest-memory`, in MiB). |
