...
**Shared — compiled once, run by all three targets (2,093 LOC)**
...
| Shell and protocol routing (§12.10) | `host/shell-core.ts` | 347 |
...
| Target | What | LOC |
| --- | --- | --- |
| **JS** (browser + Node) | sockets (TCP/WS/WebRTC), the `fs` backend, safe-js realms, worker-backed pure modules, PQ drivers, entry points, key derivation | 1,535 TS |
| **Native** (Go) | QuickJS embedding, event loop, libsodium and pure modules over wazero, raw net and fs — plus `native-shim.ts` (401) and `native-polyfills.ts` (93), both TypeScript and riding in the shared bundle | 2,152 Go + 494 TS |
