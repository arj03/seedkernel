...
**Shared — compiled once, run by all three targets (2,394 LOC)**
...
| Shell and protocol routing (§12.10) | `host/shell-core.ts` | 414 |
...
| Core seam and vocabulary — the socket/`fs` contracts, the key space and flood bounds, domain prefixes, the master-seed subkey derivation (§12.6.2b), the manifest suite id, the host-call names, the app-facing op envelope | `core/*.ts` (8 files) | 343 |
...
| Target | What | LOC |
| --- | --- | --- |
| **JS** (browser + Node) | sockets (TCP/WS/WebRTC), the `fs` backend, safe-js realms, worker-backed pure modules, manifest-verifier plumbing, entry points, key derivation | 1,448 TS |
| **Native** (Go) | QuickJS embedding, event loop, libsodium and pure modules over wazero, raw net and fs — plus `native-shim.ts` (375) and `native-polyfills.ts` (93), both TypeScript and riding in the shared bundle | 2,002 Go + 468 TS |
