...
**Shared — compiled once, run by all three targets (2,481 LOC)**
...
| Target | What | LOC |
| --- | --- | --- |
| **JS** (browser + Node) | sockets (TCP/WS/WebRTC), the `fs` backend, safe-js realms, worker-backed pure modules, manifest-verifier plumbing, entry points, key derivation | 1,498 TS |
| **Native** (Go) | QuickJS embedding, event loop, libsodium and pure modules over wazero, raw net and fs — plus `native-shim.ts` (415) and `native-polyfills.ts` (93), both TypeScript and riding in the shared bundle | 2,159 Go + 508 TS |
