...
- Untrusted code is **bounded** as well as confined. A WASM handler declares its linear-memory ceiling in its signed manifest and the loader refuses anything unbounded or over budget; a JS guest runs under a heap cap and an execution-time budget. Both default to real numbers, so a shell that configures neither still gets bounded code. The one gap is a WASM handler's CPU time, where the JS platform offers no mechanism to call (§4.3, §14).
...
| --- | --- | --- |
| **Shared logic** — the transport, PeerLink and routing (§12.6); the bundle format, admission policy and handler resource bounds (§12.4, §12.5, §4.1); the cap-bridge and the guest ABI preamble (§12.2); the WebSocket codec; and `createShell`, the one assembly of all of it | 16 `.ts` files in `WASM/host/`, compiled into one bundle — the native loader embeds that same output and runs it in QuickJS (§12.9) | ~3,300 lines |
...
| **Per-target host** — sockets, files, the crypto FFI, the JS engine embedding, the event loop, the confined realm | `native/*.go` | ~3,400 lines of Go |
...
To read the spec as one document, concatenate the files in that order: `cat README.md docs/{PROTOCOL,RUNTIME,SECURITY}.md`. CHANNEL is rationale rather than spec and sits outside that sequence.
