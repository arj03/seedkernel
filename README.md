...
**Shared — compiled once, run by all three targets (2,441 LOC)**
...
| Guest seam — the guest ABI seam (§12.2) | `host/guest-seam.ts`, `host/realm-queue.ts` | 518 |
...
| **Native** (Go) | QuickJS embedding, event loop, libsodium and pure modules over wazero, raw net and fs — plus `native-shim.ts` (444) and `native-polyfills.ts` (93), both TypeScript and riding in the shared bundle | 2,159 Go + 537 TS |
