...
- **Every host-side byte caused by untrusted input has an owner.** Confinement answers what a guest may *reach*, not how much of the node an untrusted **peer** can make the host hold — and the host holds a peer's bytes long before any guest sees them. So one rule replaces a list of defences: every retained byte is charged to a finite owner from creation until destruction, and when data moves the receiving owner reserves before the previous one releases. The consequences are the unobvious part — a link's outbound custody is **one** period spanning the adapter's pre-open queue and the platform's send backlog, an inbound read stays charged until the serialized realm answers it, and the node's total is summed against a real machine in a test rather than trusted per owner (§12.3, §12.6).
...
**Shared — compiled once, run by all three targets (2,360 LOC)**
...
| Shell, node assembly and claim routing (§12.9, §12.10) | `host/shell-core.ts` | 448 |
...
| Target | What | LOC |
| --- | --- | --- |
| **JS** (browser + Node) | sockets (TCP/WS/WebRTC), the `fs` backend, safe-js realms, worker-backed pure modules, manifest-verifier plumbing, entry points, key derivation | 1,489 TS |
| **Native** (Go) | QuickJS embedding, event loop, libsodium and pure modules over wazero, raw net and fs — plus `native-shim.ts` (413) and `native-polyfills.ts` (93), both TypeScript and riding in the shared bundle | 2,117 Go + 506 TS |
