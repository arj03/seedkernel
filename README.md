...
**Shared — compiled once, run by all three targets (2,371 LOC)**
...
| Target | What | LOC |
| --- | --- | --- |
| **JS** (browser + Node) | sockets (TCP/WS/WebRTC), the `fs` backend, safe-js realms, worker-backed pure modules, PQ drivers, entry points, key derivation | 1,538 TS |
| **Native** (Go) | QuickJS embedding, event loop, libsodium and pure modules over wazero, raw net and fs — plus `native-shim.ts` (389) and `native-polyfills.ts` (93), both TypeScript and riding in the shared bundle | 2,144 Go + 482 TS |
