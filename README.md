...
| --- | --- | --- |
| **Shared logic** — the transport, PeerLink and routing (§12.6); the bundle format and admission policy (§12.4, §12.5); the cap-bridge and the guest ABI preamble (§12.2); the WebSocket codec; and `createShell`, the one assembly of all of it | 15 `.ts` files in `WASM/host/`, compiled into one bundle — the native loader embeds that same output and runs it in QuickJS (§12.9) | ~3,100 lines |
