...
**Shared — compiled once, run by all three targets (2,472 LOC)**
...
| Core seam and vocabulary — the socket/`fs` contracts, the key space and flood bounds, domain prefixes, the master-seed subkey derivation (§12.6.2b), the manifest suite id and the host-call names | `core/*.ts` (7 files) | 296 |
...
| **Native** (Go) | QuickJS embedding, event loop, libsodium and pure modules over wazero, raw net and fs — plus `native-shim.ts` (428) and `native-polyfills.ts` (93), both TypeScript and riding in the shared bundle | 2,108 Go + 521 TS |
